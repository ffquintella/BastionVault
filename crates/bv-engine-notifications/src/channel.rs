...
//! `Arc<dyn VaultCtx>` and reaches the mount table, so it sits above this
//! engine in the crate graph. See roadmaps/workspace-decomposition.md
//! § Phase 3.
//!
...
pub async fn list_channels(core: Arc<dyn VaultCtx>) -> Result<Vec<ChannelInfo>, RvError> {
    ...
    // No plugin runtime registered (a minimal embedded build) means no plugin
    // channels — the in-app channel above is still listed.
    if let Some(host) = core.plugin_host() {
        for ch in host.notification_channels().await {
            out.push(ChannelInfo {
                id: format!("{}:{}", ch.plugin, ch.id),
                name: ch.name,
                kind: ch.kind,
                description: ch.description,
                provider: format!("plugin:{}", ch.plugin),
                enabled: true,
            });
        }
    }
...
pub async fn deliver_to_channel(
    ...
    let Some(host) = core.plugin_host() else {
        return err_result(channel_id, total, "no plugin runtime on this server".to_string());
    };

    match host.invoke(plugin_name, &input).await {
        Ok(output) => {
            if let Some(code) = output.error_status {
