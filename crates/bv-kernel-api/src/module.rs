//! The [`Module`] trait: the lifecycle contract every BastionVault engine
//! implements.
//!
//! It moved out of `bastion_vault::modules` in Phase 3 and is re-exported from
//! there, so `bastion_vault::modules::Module` still resolves. It had to move
//! for the plainest possible reason: an engine crate cannot implement a trait
//! defined in the crate above it. Every Tier 3 engine implements this, so
//! nothing could leave the monolith while it lived there.
//!
...
use crate::{services::KernelServices, ctx::VaultCtx};
