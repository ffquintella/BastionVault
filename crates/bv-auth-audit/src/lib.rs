...
// The substrate, under the names this file has always spelled it.
...
impl LoginAuditStore {
    ...
    pub async fn list_since(&self, since_key: &str) -> Result<Vec<LoginAuditEntry>, RvError> {
    ...
    fn decode(mut entries: Vec<StorageEntry>) -> Vec<LoginAuditEntry> {
        entries.sort_by(|a, b| b.key.cmp(&a.key));
        entries.into_iter().filter_map(|e| serde_json::from_slice::<LoginAuditEntry>(&e.value).ok()).collect()
    }
}
