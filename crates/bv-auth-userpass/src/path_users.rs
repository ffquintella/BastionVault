...
/// `pub` so the relocated lockout tests can compute the same epoch the entry
/// stores.
pub fn now_secs() -> i64 {
...
/// (`bastion_vault::modules::system::self_profile`) so an address an admin may
/// store for a user is exactly one the user may store for themselves. `pub`
/// rather than `pub(crate)` since that route is now in a different crate --
/// two copies of this predicate is how the two would drift apart.
