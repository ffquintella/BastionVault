...
/// deleted.
/// `pub` so the relocated integration tests can assert against it: they had
/// to leave this crate (they stand up a whole vault) and now name it through
/// `bastion_vault::modules::files`.
pub const DEFAULT_VERSION_RETENTION: usize = 5;
...
/// `pub` for the same reason as [`DEFAULT_VERSION_RETENTION`].
...
fn dispatch_push(target: &FileSyncTarget, bytes: &[u8]) -> Result<(), RvError> {
    match target.kind.as_str() {
        ...
        "smb" => {
            #[cfg(feature = "files_smb")]
            {
                crate::smb::push_smb(target, bytes)
            }
        ...
        "sftp" => {
            #[cfg(feature = "files_ssh_sync")]
            {
                crate::ssh_sync::push_sftp(target, bytes)
            }
        ...
        "scp" => {
            #[cfg(feature = "files_ssh_sync")]
            {
                crate::ssh_sync::push_scp(target, bytes)
            }
            #[cfg(not(feature = "files_ssh_sync"))]
            {
                Err(RvError::ErrString(
