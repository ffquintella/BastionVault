...
/// The `sys/init` request body. Defined in `bastion_vault::api::sys`, next to
/// the client that serializes it, and re-exported here so this module's own
/// paths are unchanged. See Phase 4 of the decomposition roadmap.
pub use bastion_vault::api::sys::InitRequest;
...
async fn fetch_backup_from_node(
    ...
    let body = tokio::task::spawn_blocking(move || -> Result<Vec<u8>, String> {
        ...
        let resp = agent
            .get(&url)
            .header(crate::VAULT_AUTH_HEADER_NAME, &token)
            .call()
