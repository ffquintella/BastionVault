...
mod test {
    ...
    use crate::test_support::{new_test_backend, new_test_temp_dir};
    ...
    // `test_file_backend_multi_routine` lives in the root crate
    // (`src/storage_backend_tests.rs`): it drives a second vault process
    // through the `bvault` CLI harness, which this crate does not have.
