...
// `pub` rather than `pub(crate)`: the CLI's `operator cloud-target rekey`
// command reads both of these, and it now lives in a different crate.
...
pub fn encode_manifest(keys: &[String]) -> Vec<u8> {
