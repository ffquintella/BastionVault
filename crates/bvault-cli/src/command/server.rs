...
// The HTTP surface and its two actix middleware layers live in `bv-server`
// as of Phase 4. This command is the wrap site — the one place that assembles
// an actix `App` out of them.
use bv_server as http;
use bv_server::middleware::{dos::dos_middleware, metrics::metrics_midleware};

use crate::kernel_api::VaultCtx;
use crate::{
    command, config, config::TlsVersion,
    errors::RvError,
...
impl Server {
    ...
    pub fn main(&self, config_path: &PathBuf) -> Result<(), RvError> {
        ...
        // the gate.
        let (metrics_access, bad_metrics_cidrs) = http::metrics_routes::MetricsAccess::parse(
