...
impl RustionBackendInner {
    ...
    /// `pub` because the namespace-scoping tests that pin it — a missing
    /// header must resolve root-relative, an empty one must not be treated as
    /// a namespace, and a named one must scope every sub-request — had to move
    /// to the root crate, where `test_utils` can stand up a vault.
    pub async fn namespace_sub_request_prefix(&self, req: &Request) -> Result<String, RvError> {
