...
// `pub` only so the lifted test block in `bastion_vault::engine_tests` can
// reach it; see roadmaps/workspace-decomposition.md § Phase 4.5.
...
// `pub` only so the lifted test block in `bastion_vault::engine_tests` can
// reach it; see roadmaps/workspace-decomposition.md § Phase 4.5.
pub const ACTIVATION_MARKER_KEY: &str = "reroot-active";
...
// `pub` only so the lifted test block in `bastion_vault::engine_tests` can
// reach it; see roadmaps/workspace-decomposition.md § Phase 4.5.
pub async fn activation_active(barrier: &dyn Storage) -> Result<bool, RvError> {
...
// `pub` only so the lifted test block in `bastion_vault::engine_tests` can
// reach it; see roadmaps/workspace-decomposition.md § Phase 4.5.
pub fn reroot_destination(root_uuid: &str, key: &str) -> Option<String> {
