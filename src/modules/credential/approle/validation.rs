...
pub struct SecretIdStorageEntry {
    ...
    // environments is the set of environment glob patterns this secret_id is
    // permitted to access (matched against the `env` request parameter on KV v2
    // reads/writes). An empty list means the secret_id is not environment
    // scoped and may access all environments. Wildcards are allowed (e.g.
    // "prod-*").
    #[serde(default)]
