...
impl SshSignAuditStore {
    ...
    pub async fn list_since(&self, since_key: &str) -> Result<Vec<SshSignAuditEntry>, RvError> {
    ...
    fn decode(mut entries: Vec<StorageEntry>) -> Vec<SshSignAuditEntry> {
        entries.sort_by(|a, b| b.key.cmp(&a.key));
        entries.into_iter().filter_map(|e| serde_json::from_slice::<SshSignAuditEntry>(&e.value).ok()).collect()
    }
}
