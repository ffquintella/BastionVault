...
impl FileAuditStore {
    ...
    pub async fn list_since(&self, since_key: &str) -> Result<Vec<FileAuditEntry>, RvError> {
    ...
    fn decode(mut entries: Vec<StorageEntry>) -> Vec<FileAuditEntry> {
        entries.sort_by(|a, b| b.key.cmp(&a.key));
        entries.into_iter().filter_map(|e| serde_json::from_slice::<FileAuditEntry>(&e.value).ok()).collect()
    }
}
