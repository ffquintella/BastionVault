...
**Inbound CSR decisions live in a sibling spec.** `pki/sign-request/*` — import
a CSR generated elsewhere, dry-run it against every role, approve or refuse it
on the record — is specified in
[pki-inbound-sign-requests.md](pki-inbound-sign-requests.md). It also owns
`sign_flow.rs`, the shared decide-then-build pipeline that `pki/sign/:role` and
`pki/sign-verbatim` (Phase 5, below) now run through.

**Phases 1 + 2 + 3 + 4 + 4.1 + 5 + 5.1 + 5.2 + 5.3 + 5.4 + 5.5 + 5.6 + 5.7 — Done.** Phase 5.7 adds the **Windows / AD smart-card logon certificate profile** — UPN `otherName` SANs, arbitrary EKU OIDs (incl. the `smartcardlogon` alias), and the `szOID_NTDS_CA_SECURITY_EXT` SID extension required for KB5014754 strong mapping — plus wires `pki/config/urls`' CRL distribution points into issued leaves for the first time. Every knob is closed by default, and the SID is configurable both as a role default and per request. See the Phase 5.7 section below for why the SID extension (rather than an `altSecurityIdentities` mapping) is the only workable route for per-session certs. Phase 5.6 turns `pki/config/ca` into a full **chain importer**: a single paste may carry one or more CA certificates plus an optional private key. The cert whose public key matches the key becomes a signing issuer (order-independent); every other CA cert imports as a **key-less** (trust/chain-only) issuer — a new issuer shape (`CaMetadata.keyless`, `IssuerHandle.signer: Option<Signer>`) that resolves `ca_chain` and lists like any issuer but is rejected at signing time (`issuers::require_key`). With no key at all, every CA imports as a trust anchor. Re-importing a cert already at the mount (matched by serial) is an idempotent skip, and import-validation failures are now HTTP 400 with actionable messages instead of the old blanket `HTTP 500: PKI pem bundle is invalid`. The GUI Import-root-CA modal renders a live chain tree while pasting and the imported hierarchy on success (new local-only `pki_parse_chain` Tauri command). Phase 5.5 adds Vault-shape per-issuer `usage` flags (`issuing-certificates` / `crl-signing` / `ocsp-signing`) so an operator can dedicate one issuer to issuing leaves and another to CRL signing. Default behaviour preserved (legacy issuers and freshly-created ones both carry all usages enabled). Phase 5.4 unifies the three CRL builders (classical / PQC / composite) onto a single shared helper, sharing the `x509-cert::crl` machinery. Pure refactor — no behavioural change visible to clients. Future CRL-extension additions land in one place. Phase 5.3 swaps the caller-facing PQC private-key format from the engine-internal `BV PQC SIGNER` envelope to standard PKCS#8 `PrivateKeyInfo` (per the IETF lamps draft) and lights up RSA generation by routing the `rsa` crate's keygen → PKCS#8 → `rcgen::KeyPair::from_pem_and_sign_algo`. ~~~~ Storage stays on the internal envelope, so existing on-disk PQC keys read cleanly without migration. Phase 5.1 closed the CSR-signing asymmetry — PQC roles can now sign client-supplied CSRs via `pki/sign/:role` and `pki/sign-verbatim`, not just generate engine-side keypairs via `pki/issue`. Phase 5.2 closed the single-issuer-per-mount call-out from Phase 1's "Not In Scope" list — each mount can now hold multiple named issuers, with a default-issuer pointer, role-level `issuer_ref` pinning, request-body override, per-issuer CRL state, full lifecycle (rename + delete with default-pointer guard), and a lazy migration shim that lifts pre-5.2 mounts on first read. Phase 3 ships behind the `pki_pqc_composite` cargo feature flag (off by default) since the IETF draft for composite signatures is still moving. Phase 4 + 4.1 ship the full `pki/tidy` story: on-demand `pki/tidy`, `pki/tidy-status`, `pki/config/auto-tidy`, *and* the periodic scheduler. Phase 5 ships CSR-driven signing (`pki/sign/:role`, `pki/sign-verbatim`), intermediate-CA hierarchies (`pki/intermediate/generate`, `pki/intermediate/set-signed`, `pki/root/sign-intermediate`), and external CA import (`pki/config/ca`). ACME server endpoints have been spun out as a separate future feature — see [pki-acme.md](pki-acme.md). Phase 1 ships the pure-Rust classical engine (RSA / ECDSA P-256/P-384 / Ed25519) on `rcgen` 0.14 with the `ring` provider; Phase 2 layers on ML-DSA-44/65/87 PQC roles built directly on `x509-cert` + `der` + `fips204`, sidestepping rcgen for PQC because rcgen's ML-DSA support is gated behind `aws_lc_rs_unstable` (forbidden). No `openssl-sys` or `aws-lc-sys` is pulled in by this module across either phase. The legacy OpenSSL-bound `path_*.rs` files have been removed; the new module lives in flat layout at `src/modules/pki/`:
...
- [`issuers.rs`](../src/modules/pki/issuers.rs) — Phase 5.2 multi-issuer storage layer + lazy migration. `IssuerHandle`, `load_default_issuer`, `load_issuer`, `add_issuer`, `rename_issuer`, `delete_issuer`, plus the migration shim that lifts legacy `ca/*` singletons into `issuers/<uuid>/*` on first multi-issuer read.
- [`path_issuers.rs`](../src/modules/pki/path_issuers.rs) — Phase 5.2 routes: `pki/issuers` (list), `pki/issuer/:ref` (read/write/delete), `pki/config/issuers` (default pointer).
...
- [tests/test_pki_phase5.rs](../tests/test_pki_phase5.rs) — Phase 5 (3 tests): (a) `sign/:role` + `sign-verbatim` happy-path with a locally-built rcgen CSR, plus tampered-CSR rejection; (b) full intermediate hierarchy across two PKI mounts (root + intermediate), confirms leaf chains through both certs to root; (c) `config/ca` import happy-path, idempotent same-bundle re-import (no-op skip), and rejection of a mismatched cert/key bundle.
- [tests/test_pki_chain_import.rs](../tests/test_pki_chain_import.rs) — Phase 5.6 chain import (5 cases): key + full chain in one paste (int becomes signing issuer, root a key-less anchor, leaf chains through both); certs-only trust import (all key-less, no default, issuance refused); key matching no pasted cert rejected; leaf in the bundle rejected; idempotent re-import of a shared root (skipped by serial).
- [tests/test_pki_phase5_1.rs](../tests/test_pki_phase5_1.rs) — Phase 5.1 PQC CSR signing: hand-assembles an ML-DSA-65 CSR, self-test-verifies with `fips204`, submits to `pki/sign/:role` on a PQC mount; asserts leaf OID + SPKI bytes; tampered CSR rejected; mixed-chain rejected.
- [tests/test_pki_phase5_2.rs](../tests/test_pki_phase5_2.rs) — Phase 5.2 multi-issuer (4 cases): two issuers + default swap; `issue/:role` issuer_ref priority + per-issuer CRL isolation; rename + delete (with default-pointer guard); legacy migration shim observable.
- [tests/test_pki_phase5_3.rs](../tests/test_pki_phase5_3.rs) — Phase 5.3 PKCS#8 + RSA (2 cases): (a) PQC PKCS#8 round-trip — issue ML-DSA-65 leaf, decode the returned PrivateKeyInfo, verify the inner OCTET STRING contains the 32-byte seed, run it through `fips204` to confirm the regenerated public key matches the cert's SPKI; (b) RSA-2048 root + leaf, confirm `signatureAlgorithm` OID is `sha256WithRSAEncryption` (bit-size→hash convention).
...
**GUI file-based key + cert import.** The Keys tab's "Import managed key" modal takes either a pasted PEM key or a **PKCS#12** (`.p12`/`.pfx`) file — the container is unwrapped locally in the Tauri process (passphrase never leaves the machine), only the private key is grabbed (PKCS#8) and sent to `pki/keys/import`; certs in the container are ignored. The Certificates tab gains an **Import** button whose modal accepts **PEM / PKCS#7 (`.p7b`) / PKCS#12** files (PEM may also be pasted): every certificate in the file is indexed into the orphan-cert store (`pki/certs/import`) independently, with a per-cert imported/already-present/failed tally. New Tauri command `pki_import_certs_file`; the PKCS#12 unwrap (`unwrap_pkcs12`) is shared with the Import-root-CA path and PKCS#7 is parsed with the same `cms` crate the export path uses. No private key is stored on the cert-import path.

**GUI certificate renewal.** The Certificates tab's detail panel gains a **Renew** action for any selected cert. It opens a modal pre-filled from the selected cert's parsed subject (CN), DNS/IP SANs, and originating `issuer_id`, and requires the operator to pick a role — the cert store persists only the resulting PEM (`CertRecord`), not the role or requested TTL that minted the serial, so renewal is an operator-confirmed re-issue rather than an automatic one. Orphan (externally-imported) certs are also renewable: they carry no issuer on this mount, so the issuer field defaults to empty (the mount's default issuer signs the renewal) and the operator can override it — this is how an imported/expired cert is turned into a fresh, valid cert with the same identity. Note the re-issue path (`pki/issue/<role>`) mints an **end-entity** cert; renewing a CA/intermediate cert this way does not reproduce a CA cert (use the intermediate-CA flow for that). Submitting calls the existing `pki_issue_cert` (fresh keypair unless a managed key is pinned via `key_ref`); the new cert, issuing CA, and one-time private key stay visible until the modal is closed. An optional toggle revokes the old serial only *after* a successful re-issue. This is a GUI-side composition of existing commands (mirroring how the cert-lifecycle module renews a managed target) — no new server route. GUI-only in `gui/src/routes/PkiPage.tsx` (`RenewCertModal`); covered by `gui/src/test/pkiRenew.test.tsx`.

**Certificate ↔ managed-key association.** A new logical route `pki/cert/<serial>/key` lets an operator bind (Write, body `{key_ref}`) or clear (Delete) the managed key that backs a stored certificate. This closes the import gap: `pki/certs/import` records a cert with an empty `key_id` and the PKCS#12 key-grab imports a key without linking it to any cert, so the two arrive unlinked and there was previously no way to associate them. The binding is **verified** — the handler parses the stored cert's `SubjectPublicKeyInfo` and compares its DER to the managed key's public SPKI (`keys::entry_spki_der`, the same byte-equality contract `pki/sign/:role` uses to pin a CSR to a key); a mismatch is rejected rather than stored, so only the cryptographically-correct key can be attached and a wrong/missing binding can be corrected after the fact. Re-pointing drops the old cert-ref and adds the new one so per-key reference counts (which gate `DELETE pki/key/<id>`) stay exact; `pki/cert/<serial>` now surfaces `key_id`/`key_name`. GUI: the Certificates detail panel shows the **Managed key** binding and offers **Associate key** / **Change key** / **Clear key** (`AssociateKeyModal` picks from the mount's managed keys; the server enforces the match). Tauri commands `pki_associate_key` / `pki_clear_cert_key`. Handlers in `src/modules/pki/path_fetch.rs` (`set_cert_key` / `clear_cert_key`); covered by `tests/test_pki_cert_key_lifecycle.rs::test_associate_key_with_certificate` and `gui/src/test/pkiKeyAssociation.test.tsx`.
...
### Phase 5.7 -- Windows / AD Smart-Card Logon Certificate Profile — **Done**

Closes the gap that made the `pki` credential source for RDP ([resource-connect.md](resource-connect.md)) unusable against a real domain: the engine could mint a cert and wrap it as a synthetic PIV smart card, but the cert itself could never map to an AD account. SANs were DNS/IP only and `ext_key_usage` mapped just the six named RFC 5280 purposes, so all three Microsoft-specific requirements were unreachable.

| File | Purpose |
|---|---|
| `src/modules/pki/ad_ext.rs` | New. The AD/Microsoft extension encoders + validators, shared by all three cert builders so the `rcgen` path and the hand-rolled DER paths cannot drift. OID constants, `upn_general_name`, `sid_extension{,_der}`, `validate_upn`, `validate_ad_sid`, `normalize_ad_sid`, and a stricter-than-`const_oid` `parse_oid` that rejects empty arcs (`311..2` would otherwise parse as a `0` arc and silently emit the wrong OID). |
| `src/modules/pki/path_roles.rs` (extension) | Role knobs: `allow_upn_sans`, `allowed_upn_domains`, `ext_key_usage_oids`, `allow_ad_sid`, `ad_sid`. OIDs and the default SID are validated at role-write time. |
| `src/modules/pki/path_issue.rs` (extension) | `upn_sans` + `ad_sid` request fields on `issue/:role` and `sign/:role`; `resolve_ad_smartcard_input` applies the role gate and the request > role > absent SID priority. |
| `src/modules/pki/x509.rs` (extension) | `SubjectInput.{upn_sans,ad_sid}`; UPN via `SanType::OtherName`, SID via `CustomExtension`; `parse_ext_key_usages` now resolves raw OIDs, `smartcardlogon`, `kdcauthentication`, `any`, plus `role.ext_key_usage_oids`. |
| `src/modules/pki/x509_pqc.rs`, `x509_composite.rs` (extension) | Same profile on the hand-assembled DER paths: `build_ad_sid_extension`, UPN otherNames folded into `build_subject_alt_name`, EKU OIDs in `build_extended_key_usage`. |

What emits, all **closed by default** so an existing role's certificate profile is unchanged:

1. **UPN as a `subjectAltName` `otherName`** — OID `1.3.6.1.4.1.311.20.2.3`, value a UTF8String (Microsoft's guidance names non-UTF8 encodings as a common cause of AD refusing an otherwise valid cert). The UPN joins the *existing* SAN extension; a second `subjectAltName` would make the cert invalid, which is why this goes through `rcgen`'s SAN list rather than a custom extension.
2. **Smart Card Logon EKU** — `1.3.6.1.4.1.311.20.2.2`, reachable as the `smartcardlogon` alias or a raw OID. `kdcauthentication` (`1.3.6.1.5.2.3.5`) is also aliased so DC certs can be minted from a BastionVault issuer.
3. **`szOID_NTDS_CA_SECURITY_EXT`** — OID `1.3.6.1.4.1.311.25.2`, carrying the account SID for KB5014754 strong mapping.

**Why the SID extension is load-bearing rather than optional.** The September 9, 2025 Windows update removed `StrongCertificateBindingEnforcement`, so patched domain controllers are permanently in Full Enforcement and deny any cert that cannot be *strongly* mapped. The documented alternative — a strong `altSecurityIdentities` mapping — is `X509IssuerSerialNumber`, `X509SKI`, or `X509SHA1PublicKey`, all of which bind to one specific certificate. Resource Connect mints a fresh cert **and** a fresh keypair per session, so every one of those values changes each connect and the AD attribute would need rewriting before every logon. Emitting the SID extension is the only route that keeps per-session issuance viable. KB5014754 also notes that non-Microsoft CAs never get this extension automatically, which is exactly the gap this closes.

Deliberately **not** emitted by `pki/sign-verbatim`, `pki/csr/generate`, or ACME: there is no role to gate the first two (and ACME identity is proven by a DNS/HTTP challenge), and an ungated SID extension is an unauthenticated identity claim.

Also in this phase: `pki/config/urls`' `crl_distribution_points` are finally embedded in issued leaves. The config had been accepted and persisted since Phase 1 but no builder read it, so every leaf shipped without a CDP and verifiers had no revocation source — and a DC validating a smart-card logon cert wants a reachable CRL. AIA (`issuing_certificates` / `ocsp_servers`) remains stored-but-unused and is a separate track.

Coverage: [tests/test_pki_ad_smartcard.rs](../tests/test_pki_ad_smartcard.rs) — 3 tests. Closed-by-default refusals; full profile on the classical path with structural DER assertions (UPN otherName type-id + UTF8String tag; SID nested as `GeneralNames { otherName { szOID_NTDS_OBJECTSID, [0] OCTET STRING } }`; exactly one `subjectAltName`; DNS SAN coexists); role-default vs request-body SID precedence; realm allow-list; malformed UPN / SID / EKU-OID rejection at both role-write and issue time; SID normalisation; CDP embedding and its absence before `config/urls` is set; and the same profile re-verified on the ML-DSA path, which is a genuinely separate DER implementation.

**Remaining before a live AD run:** the operator still has to publish the issuing CA to the forest's NTAuth store (`certutil -dspublish -f <ca>.cer NTAuthCA`), trust the root domain-wide, and ensure DCs hold a KDC-Authentication cert. The engine side is complete; AD-side enrolment is documented operator work.
