...
**GUI certificate renewal.** The Certificates tab's detail panel gains a **Renew** action for engine-issued certs (hidden for orphan imports, which carry no role/issuer on the mount). It opens a modal pre-filled from the selected cert's parsed subject (CN), DNS/IP SANs, and originating `issuer_id`, and requires the operator to pick a role — the cert store persists only the resulting PEM (`CertRecord`), not the role or requested TTL that minted the serial, so renewal is an operator-confirmed re-issue rather than an automatic one. Submitting calls the existing `pki_issue_cert` → `pki/issue/<role>` path (fresh keypair unless a managed key is pinned via `key_ref`); the new cert, issuing CA, and one-time private key stay visible until the modal is closed. An optional toggle revokes the old serial only *after* a successful re-issue. This is a GUI-side composition of existing commands (mirroring how the cert-lifecycle module renews a managed target) — no new server route. GUI-only in `gui/src/routes/PkiPage.tsx` (`RenewCertModal`); covered by `gui/src/test/pkiRenew.test.tsx`.
