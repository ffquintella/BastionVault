...
- [Resource Connect](resource-connect.md) ships seven phases (SSH × {Secret, LDAP, PKI} ✅, RDP × {Secret, LDAP, PKI smartcard via CredSSP} ✅, ⌘K palette, per-type policy). As of Phase 7.4, the in-app Connect button consults the per-resource Rustion policy and routes both SSH-password and RDP-password sessions through a bastion when transport is `rustion-required` or `rustion`; non-password SSH and smart-card (rdp-cert) RDP under `rustion-required` fail closed pending the bastion's PKINIT path.
