...
- [Resource Connect](resource-connect.md) ships seven phases (SSH × {Secret, LDAP, PKI} ✅, RDP × {Secret, LDAP, PKI smartcard via CredSSP} ✅, ⌘K palette, per-type policy). As of Phase 7.4, the in-app Connect button consults the per-resource Rustion policy and routes both SSH-password and RDP-password sessions through a bastion when transport is `rustion-required` or `rustion`; SSH key and certificate credentials now route through the bastion too (only passphrase-protected private keys still fail closed, pending a passphrase channel on the `session/open` wire). As of Phase 9.5, smart-card (rdp-cert) RDP also routes through the bastion via Kerberos PKINIT / SPNEGO (the envelope carries the cert + key + PIN; Rustion drives the upstream CredSSP with an sspi smart-card identity), so it no longer fails closed on the BastionVault side — the sspi engine is build-gated (`rustion_pkinit_sspi`) and live-AD validation is pending.
...
- **Recording playback is chunked and size-independent.** `rustion/recordings/<rid>/blob/chunk/<n>` (4 MiB slices, whole-artifact `sha256` on every chunk) is what the player reads; the single-response `/blob` route remains for existing clients but cannot carry a recording past the client's response-size limit. Chunk reads are backed by a bounded, seal-cleared in-memory artifact cache, because the bastion honours no `Range`. See Phase 6.5 below.
- **The vault verifies an artifact before it serves or caches it.** Both artifact routes hash the bytes the bastion returned against the digest on record (the sidecar's `sha256` in preference to the response header) and refuse a mismatch with a `409` + `recording.artifact.rejected`, so the playback path now enforces what the keystroke indexer has always enforced before persisting derived text. Because the check runs before the cache insert, a chunked read cannot serve a corrupted entry a slice at a time. A recording whose sidecar carries no digest still plays, with `digest_verified: false` on the response. See Phase 6.5 below.
...
**Corollary — a bastion's `401 signature_invalid` is never a namespace fault.** The master keypair is read from the root system view (`MasterStore::new` → `core.state.system_view`), the store is created once per unseal on the deployment-global `RustionModule`, and `rustion/` is header-scoped rather than path-rewritten — so a namespaced session/open signs its envelope with byte-identical key material to a root one. When a bastion answers `401 {"error":"signature_invalid","detail":"Ed25519 signature half failed verification"}`, it is telling us the pubkey it pinned when the `bastion-vault` authority was approved is not the pubkey we sign with now (master rotated or re-issued without re-approval on that bastion; or no PKI-issued master at all, so `get_or_init_signing_key` minted one on the fly with a `legacy-…` serial that no bastion has ever seen). Because the refusal arrives as a caller-facing `401` on a namespaced request, it reads like a namespace-scoping or token bug, and repeatedly did. `master::signature_rejection_hint` now appends the serial + fingerprint actually used, the never-PKI-issued and rotated-but-not-propagated cases, and the `GET rustion/master/pubkey` comparison to run — wired into session open/renew/kill and `authority/attest`. Note that rotation is a **hard cutover** for the fleet: `attest` envelopes are signed with the current key too, so there is no in-band way to refresh a bastion's pin; each bastion must re-approve. Covered by six tests in `modules::rustion::master::tests` (`hint_*`).
...
- **`RecordingPlayerModal`**: opens on row click. Loads bytes via `fetchRecordingBytes` (chunked — see below), dispatches to a format-specific renderer.
- **Chunked artifact reads — playback at any recording size** (added after the fact, when a 17.8 MB RDP recording proved unplayable): `GET rustion/recordings/<rid>/blob/chunk/<n>` serves one 4 MiB slice plus `size_bytes` / `chunk_count` / `chunk_size` / `offset` / `chunk_len` / `eof`, with the whole-artifact `sha256` repeated on every chunk so the assembled bytes are verified exactly as before. The single-shot `/blob` route put the entire artifact in one JSON response, base64-expanded (17.8 MB → ~23.7 MB), which a client with a response-size limit cannot read at all — ureq's default is 10 MB, and that is what the GUI reported as ``node `…:5200` is unavailable``. 4 MiB per chunk is chosen so a chunk body (~5.6 MB) stays under that default. `/blob` is unchanged and still served.
  - **Why the vault caches the artifact**: the bastion's `GET /v1/recordings/<rid>/blob` serves whole files and honours no `Range` (the same constraint that forces keystroke indexing to persist derived text), so chunk `n` cannot be fetched from upstream on its own. A bounded read-through cache in `bv-engine-rustion/src/blob_cache.rs` — 4 artifacts, 512 MiB, 180 s idle, LRU — makes one playback cost one upstream fetch instead of one per chunk. A recording artifact is immutable, so a cached copy is never stale. The bytes are recording plaintext: `Zeroizing`, never spilled to disk, dropped on seal. An artifact over the whole budget is served uncached, with a warning naming the cost.
  - **The artifact is verified before it is served or cached** (added with the chunk route's first follow-up, and deliberately separate from it — a transport fix and a behaviour change do not belong in one commit). `recordings::fetch_blob_cached` hashes the fetched bytes and compares them against the digest on record before the artifact enters the cache; `recordings::fetch_blob` itself stays a raw transfer, and its doc comment says so, because `keystroke_index` needs to report a mismatch as an index *state* (`digest-mismatch`) rather than as an error. Details that are decisions, not implementation:
    - **The sidecar's `sha256` beats the `x-recording-sha256` header, with no fallback when they disagree.** The sidecar arrived over the signed `recording.ready` webhook, verified against the bastion's pinned key; the header comes from the same party, in the same response, as the bytes it describes. Falling back would let the serving side pick the digest it is checked against.
    - **The gate precedes the cache insert, and a cache hit is not re-hashed.** That ordering is the whole point for chunked playback: chunk 0 pays for the check, every later chunk is served from an already-checked entry, and re-verifying per chunk would hash the whole artifact once per chunk to re-prove the same thing.
    - **A mismatch is a `409`, not the `502` a failed upstream fetch produces.** It is deterministic — retrying gets the same bytes — and it calls for a different operator response than an unreachable bastion. It emits `recording.artifact.rejected`, which is not `recording.replayed`: an auditor asking "who watched what" must not see refused reads in the answer.
    - **Migration**: the hard failure applies only when a digest is known and non-empty, so recordings whose sidecar landed without one are not newly unplayable. `digest_verified` on both routes distinguishes "checked" from "nothing to check against", so the compatibility case is visible rather than a silent pass. The GUI keeps its own post-assembly check as defence in depth for the vault→GUI leg, which no server-side gate can cover.
  - **Version skew is handled in one direction only**: a GUI newer than its server falls back to `/blob` when the chunk route reports `Logical backend path not supported.`, logging that it did (`isRouteUnsupported` in `gui/src/lib/error.ts`, same pattern as the Phase 7 policy routes). Small recordings then behave exactly as before; a large one still fails on the old server, which is the upgrade this route delivers. An older GUI against a newer server is unaffected — `/blob` is untouched.
  - New Tauri command `rustion_recording_blob_chunk(rid, n)`; `fetchRecordingBytes` in `gui/src/lib/rustion.ts` allocates once from chunk 0, decodes each chunk into place, reports progress, aborts the remaining chunks when the modal/window closes, and throws rather than returning a short artifact. Used by both `RecordingPlayerModal` and `SessionReplayWindow`.
...
- **`gui/src-tauri/src/commands/connect.rs` -- SSH Connect routes through Rustion when policy requires.** After resolving the credential, the handler consults the effective policy:
  - `transport=rustion-required` with an ssh-password, ssh-key, or ssh-cert credential → calls `rustion/session/open` (passing the policy hints) with the matching `credential_kind` + base64 `credential_material` (and, for ssh-cert, the signed OpenSSH cert text in `credential_cert`), then dials the returned `bastion_host:bastion_port` as user `operator` with the ticket as the SSH password. The bastion proxies to the real target, authenticating upstream via password / `authenticate_publickey` / `authenticate_openssh_cert` per kind. The key/cert shapes are byte-identical to the server's own brokered-mint path (`to_openssh(LineEnding::LF)` private key + engine `signed_key`).
  - `transport=rustion-preferred` with bastions available + any of the above → same path, falls back to direct on rustion failure.
  - `transport=rustion-required` with a **passphrase-protected** private key → fail closed with an explanatory error. There is no passphrase channel on the `session/open` wire and the rustion-ssh dispatcher treats keys as unencrypted, so an encrypted PEM would reach the bastion unusable. Under `rustion-preferred` this falls back to direct (the local dialler can decrypt).
...
- **`gui/src-tauri/src/commands/connect.rs` -- RDP Connect routes through Rustion when policy requires (BV 0.8.22).** Same shape as SSH:
  - `rustion-required` + rdp-password → `rustion/session/open` with `target_protocol=rdp` / `credential_kind=rdp-password`; dials bastion `host:port` with the ticket carried in the X.224 **cookie** slot as `Cookie: mstshash=tkt_<hex>` (via `NegoRequestData::cookie`, which writes that prefix itself — pass the bare ticket). Not the routing-token slot (`Cookie: msts=`): MS-RDPBCGR allows either, but Rustion's `extract_username_from_cookie` scans only for `mstshash=`, so a routing token is invisible to it and the bastion rejects the session as `user=unknown` without sending an RDP error PDU. The bastion consumes the ticket at the Connection Request stage, skips client-side NLA, and drives upstream CredSSP itself with the envelope's credential (Phase 4.2-full bv_credssp injection driver).
...
- **Follow-up (2026-08-24) — the pins had no refresh affordance.** Discovery ran once, best-effort, inside `handle_target_create` and nowhere else: `POST rustion/targets/<id>/listeners/refresh` and the Tauri `rustion_target_refresh_listeners` command both shipped, but no GUI control and no CLI subcommand ever called them. So a target enrolled against a pre-v2 bastion stored empty pins permanently, and the amber badge was indistinguishable from a bastion that genuinely cannot be pinned — while `resolve_bastion_dial_coords` silently degraded the hop to unpinned TOFU / unverified RDP TLS. Observed live on the FGV dev bastion, enrolled 2026-07-10 and upgraded since: the record held empty pins while `GET /v1/listeners` returned `schema_version: 2` with both. Fixed by a per-row **Re-sync** button in `RustionBastionsTab` (`rustionTargetRefreshListeners` wrapper in `gui/src/lib/rustion.ts`), plus the `listeners_synced_at` timestamp in the unpinned-badge tooltip so a stale capture is legible. Regression test: `gui/src/test/rustionListenerResync.test.tsx`.
...
- **The digest is verified before anything is persisted.** Indexing derives
  ...
  stores nothing. The playback path now runs the same check before it serves
  or caches an artifact (Phase 6.5), so the two agree on what a trustworthy
  artifact is; it reports a mismatch as a `409` rather than as a state,
  because a read either returns the bytes or does not.
...
- **How does a *namespace-bound* token reach the root `rustion/` endpoints? — Resolved: option 1, with v1 deliberately excluded.** The namespace credential split above (see *Namespaces* under Design) makes a namespaced session resolve the right credential, but it did not answer who may ask for one. A token whose login namespace is non-root resolves its named policies from that namespace's own keyspace, and `refuse_cross_namespace_paths` refuses a namespace policy that references `rustion/*` (root owns that path) — so brokered Connect worked for **root-bound** tokens only. The implicit `namespace-self` policy (`src/modules/policy/policy_store.rs`) now carries the grants:
  - **Read-only resolvers** — `rustion/policy/effective` and `rustion/dispatcher/preview`. These turned out to be load-bearing, not merely convenient: without them the GUI could not tell a brokered resource from a direct-dial one, the Connection tab's policy card reported a denied read as "not configured", and `read_effective_policy` read the 403 as "no policy" and **dialled direct** — so `rustion-required` was invisible *and* unenforced for every tenant. That fallback now fails closed (a 403 refuses the dial), which is only safe *because* the grant exists.
  - **`read` on `rustion/targets/+`** — the brokered dial verifies the bastion's SSH host-key fingerprint / RDP TLS leaf digest off the target record; without it a tenant's bastion hop silently degrades to unpinned TOFU. Single-segment wildcard, and LIST over `targets/` stays withheld, so a tenant resolves only the bastions the dispatcher already named.
  - **Session lifecycle** — `rustion/session/open` and `rustion/v2/session/open` plus `session/renew` / `session/kill`. Both open routes apply a per-resource gate (namespace-qualified), so the grants reach no resource the caller could not already reach; renew/kill additionally require the `correlation_id` handed only to the opener.

  **Option 3 landed alongside it**, because option 1 alone left half the matrix closed: v1 `session/open` is the route brokered RDP and the client-resolved SSH kinds (LDAP / PKI / FIDO2) still take, and it had no per-resource gate — it authorized the endpoint path, then brokered to a caller-named `target_host` with caller-supplied credential material, i.e. a general-purpose fleet proxy for anyone holding `update`. It now runs the same gate v2 does whenever `resource_id` is set (which every Resource Connect caller sends). Its **unbound** shape — no `resource_id`, arbitrary host, caller-supplied credential, nothing to authorize against — requires `sudo` on the path, so root and `administrator` keep the raw primitive (the `tests/e2e/rustion-ssh` harness uses it) while the tenant baseline cannot reach it. The shared engine moved to `brokered_session_open`, which authorizes nothing itself, so neither entry point can reach it ungated and v2 does not double-check.

  **The gate's own blind spot, fixed at the same time.** It probed only `ACL::explain_capability`, whose identity-less dry-run makes scope-gated rules contribute nothing — right for "does an explicit ungated grant exist?", and precisely what stops the baseline `path "resources/*" { scopes = ["shared"] }` rule from reading as a blanket grant. But it was the *only* check, so any caller whose access to a resource comes from **owning** it or from a **share** was refused at the gate: on a `rustion-required` resource they could not connect at all, and on any other they were pushed onto the direct dial. That covered most of a tenant's own inventory and every shared resource. The gate now falls back to `PolicyStore::readable_targets`, which re-runs the real evaluator against a synthesized per-object request with the owner / share / asset-group qualifiers hydrated. No escalation: such a caller can already read the stored credential through the same grant, so brokering it is strictly the safer route.

  Option 2 (an explicit per-namespace opt-in, e.g. `rustion.allow_namespace_tokens` on the global tier, so a root admin decides which tenants may broker their own sessions) remains open as a defence-in-depth follow-up.
- **The authority name BV presents is a hardcoded constant, so two BV deployments collide on one bastion record — Open.** Every outbound call sends `X-Rustion-Authority: bastion-vault` (`session.rs`, `enrolment.rs`, `probe.rs`'s `PROBE_AUTHORITY`), and Rustion resolves the *record* by that name (`crates/rustion-control-plane/src/routes.rs::require_authority`) then verifies the envelope against that record's pinned pubkey. One bastion therefore has exactly one slot named `bastion-vault`, whichever deployment claims it first: enrol a second BV (HML alongside DSV, a rebuilt cluster, a restored-from-backup vault with a re-issued master) against the same bastion and every envelope from the loser fails `401 signature_invalid`. It cannot fail any *more* specifically, because the signature is checked before the ciphertext is opened, so the `deployment_id` binding — which exists precisely to catch a different BV reusing an authority — never gets to report `attestation_mismatch`. Observed live: `bastion-vault` declared with two different pubkeys in two environments' Puppet app data. Options: derive the header from the deployment (`bastion-vault-<deployment_id prefix>`), make it a per-target field on the registry record set at enrolment, or have Rustion index approved authorities by pubkey and treat the header as a hint. All three are cross-repo; until one lands, operators must keep one BV deployment per bastion and re-approve on any master re-issue.
