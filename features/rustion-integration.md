...
- **Build gating.** The sspi Kerberos engine is compiled only under the `rustion_pkinit_sspi` build cfg (declared in `[lints.rust].unexpected_cfgs`), **not** a cargo feature — so `cargo check --all-features` never pulls the heavy sspi/picky fork stack (which needs ed25519-dalek 3.0-pre / rsa rc.18, conflicting with Rustion's russh/ssh-key 2.x pins). The default build extracts + validates the material and fails closed with a precise "engine not built" error. Activation is a deliberate two-step: uncomment the fork deps + mirror BastionVault's root `[patch.crates-io]` section, then build with `RUSTFLAGS="--cfg rustion_pkinit_sspi"`.
- **Tests:** BV — `build_open_rdp_cert_seals_key_pin_domain_in_extra` (envelope round-trip). Rustion — `SessionCredential::rdp_cert_*` extraction + wrong-kind guard; `SmartCardCredsspParams` field validation, SPN derivation, port/IPv6-bracket stripping.
- **Remaining (live-AD, same posture as 4.2-full):** reconcile the sspi/picky fork deps so `rustion_pkinit_sspi` compiles in Rustion; supply the upstream TLS SPKI for the CredSSP `pubKeyAuth` channel binding (the shared live-socket integration step with `rdp-password`); validate the PKINIT exchange against a real KDC / AD.
