...
- **In-app RDP session window** — new Tauri WebviewWindow with a canvas surface, driven by [`ironrdp`](https://crates.io/crates/ironrdp) (pure-Rust, MIT/Apache, Microsoft-spec-compliant) running in the Tauri host. Bitmap deltas streamed to the WebView via a binary Tauri channel; keyboard / mouse events (moves, buttons and wheel, both axes) streamed back.
...
| `SessionRdpWindow.tsx` route — canvas, key/mouse/wheel capture | `gui/src/routes/` |
...
- **No clipboard handoff.** The credential never goes through `navigator.clipboard`. Window-side keyboard input is forwarded straight to the protocol channel — the operator's local clipboard is never populated by us with the session credential. This still holds with RDP clipboard redirection ([features/rdp-clipboard-redirection.md](rdp-clipboard-redirection.md)), which moves only *operator-initiated* content and can be narrowed or turned
  off per resource: the resource's stored secret goes straight into the protocol and never onto any clipboard, in either direction.
