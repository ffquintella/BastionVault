...
- **No clipboard handoff.** The credential never goes through `navigator.clipboard`. Window-side keyboard input is forwarded straight to the protocol channel — the operator's local clipboard is never populated by us with the session credential. This still holds with RDP clipboard redirection ([features/rdp-clipboard-redirection.md](rdp-clipboard-redirection.md)), which is off by default and moves only *operator-initiated* content: the resource's stored secret goes straight into the protocol and never onto any clipboard, in either direction.
