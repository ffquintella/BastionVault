...
**Phases 1 and 2 are complete.** Phases 3 and 4 are not started.

- **The static server binary exists** — `cmd/bv-downloads-server/`, with
  `manifest.rs` (parse + validate), `render.rs` (askama → one static page)
  and `serve.rs` (the axum routes). 39 tests: manifest parsing and its closed
  enums, the render matrix, MIME mapping, the 404 surface, traversal and
  symlink refusals, and an end-to-end binary spawn in
  `cmd/bv-downloads-server/tests/downloads_server.rs`.
- **The container image exists** — `deploy/downloads/Containerfile`, two-stage
  `linux/amd64`, Wolfi base, nonroot UID 65532, ~19 MB, with
  `deploy/downloads/README.md` for operators and
  `deploy/downloads/test/smoke.sh` + `make downloads-image-test` for the
  acceptance run.
- **A fixture release is checked in** — `cmd/bv-downloads-server/fixtures/v0.4.0/`
  covers all eight artefact `kind`s with placeholder bodies and real SHA-256
  digests, regenerated by `make-fixture.py` next to it.
- **Nothing is signed yet.** `.github/workflows/downloads-image.yml.disabled`
  builds and pushes to GHCR on a tag with no `cosign sign`, no SBOM and no
  attestation, which is Phase 2's stated scope. The operator docs say so
  rather than advertising a verification path that does not exist.
- **No TLS, no arm64, no GUI banner.** Phases 3 and 4. Setting
  `BV_DOWNLOADS_TLS_*` today is a startup error rather than a silently ignored
  variable.
- The client binaries themselves are produced by the sibling spec
  ([features/packaging-client-binaries.md](packaging-client-binaries.md)); its
...
Three deliberate departures, each of which makes the surface smaller. The
design sections are left as written so the reasoning is reviewable.

1. **No `tower-http` `ServeDir`.** § Design and § Security Considerations
   describe resolving requests through `ServeDir` with `follow_symlinks(false)`
   plus explicit `..` normalisation. The implementation instead builds an
   allow-list of servable URLs at startup, from the manifest, and matches
   request paths against it byte-for-byte without percent-decoding. Manifest
   validation restricts names to `A-Za-z0-9._+-`, which is what makes that
   sound. This is strictly stronger — traversal is not filtered out, it has
   nowhere to land — and it drops a dependency.
2. **No `/static/` route; the CSS and logo are inlined.** § Design's route
   table is closed ("Anything outside that set returns `404`") while the
   container layout mounts a `static/` directory, which cannot both be true.
   The assets are compiled into the binary and inlined into the page, so the
   route table is exactly as specified; `deploy/downloads/static/` is copied
   into the image as the editable reference an operator overrides with
   `--static-dir` / `BV_DOWNLOADS_STATIC`. Both inline blocks are pinned by
   `sha256-` CSP source expressions computed at startup, so operator branding
   does not need `'unsafe-inline'`.
3. **The integration test lives in the crate, not the repository root.**
   § Phase 1 puts it at `tests/downloads_server.rs`. The repository's `tests/`
   links the full workspace graph (AGENTS.md § 4 L4); this crate depends on
   none of it. `cmd/bv-downloads-server/tests/downloads_server.rs` is the
   shape AGENTS.md § "Test locations" already has a row for.

Two smaller additions, both opt-in:

- `--verify-hashes` re-reads every artefact at startup and refuses to serve a
  manifest whose SHA-256 does not match the bytes on disk.
- `--render-only` prints the page and exits, so CI can check that a manifest
  renders before it is published.

### What the Testing Requirements section still asks for and does not have

- **Cucumber BDD scenarios** — none written. Two of the three are covered by
  other tests (the misconfigured-manifest refusal, and the signature path next
  to a Windows download); the third, "the next user load surfaces the new
  files with no restart", is **not implemented and not planned**: § Design
  says the index is generated at container start, so adding a release needs a
  `podman restart`. That is documented in `deploy/downloads/README.md`.
- **TLS smoke** and **GUI hookup** — Phases 3 and 4 respectively.
...
### Phase 1 — Static Server Binary — **complete**

| File | Purpose |
|---|---|
| `cmd/bv-downloads-server/*` | The Rust binary above. **Shipped.** |
| `deploy/downloads/static/*` | Minimal CSS + brand assets. **Shipped**, compiled into the binary and inlined (see the deviations above). |
| `cmd/bv-downloads-server/tests/downloads_server.rs` | Integration tests: spawns the binary, exercises every documented endpoint. **Shipped** (the spec said `tests/downloads_server.rs`; see deviation 3). |
| `cmd/bv-downloads-server/fixtures/v0.4.0/` | Checked-in fixture release, all eight `kind`s. **Added.** |

Acceptance: `cargo run -p bv-downloads-server -- --root ./cmd/bv-downloads-server/fixtures/v0.4.0`
serves an index page that lists every fixture file with the correct
hash and signature link. **Met** — all 8 artefacts, their manifest digests
and their `.sig` / `.pem` links are on the page, verified against
`manifest.json` and rendered in a browser.

### Phase 2 — Container Image, amd64 — **complete**

| File | Purpose |
|---|---|
| `deploy/downloads/Containerfile` | Two-stage build for `linux/amd64`. **Shipped** — Wolfi base, nonroot 65532, ~19 MB, builder cross-compiles from the build host's arch. |
| `deploy/downloads/README.md` | Operator docs: directory layout, mounted volumes, TLS env-var contract. **Shipped.** |
| `.github/workflows/downloads-image.yml` | Build + push on tag. **Not yet signed.** **Shipped as `downloads-image.yml.disabled`** — every image workflow in this repository carries that suffix (commit 8ba5d8c); re-enable by renaming. |
| `deploy/downloads/test/smoke.sh` + `make downloads-image{,-run,-test}` | The acceptance run, scripted. **Added.** |
...
MIME. **Met** — `make downloads-image-test` passes: the index renders in a
browser with no console errors, `/manifest.json` is byte-identical to the
mounted file, all 8 artefacts plus their `.sig` / `.pem` return their pinned
MIME and matching bytes, every undocumented path and traversal returns 404,
the four write verbs return 404, and a manifest naming a missing file refuses
to start. Run against a `linux/amd64` image on an arm64 host; the artefacts
are the checked-in placeholder fixtures, which are **not** really signed.

### Phase 3 — Multi-Arch + Signing + SBOM + TLS — *not started*
...
### Phase 4 — GUI Update Channel Hookup — *not started*
