...
**Done — every phase shipped.** Phases 1–4 (core engine + GUI). Phase 5 (SMB) behind `files_smb` (`smolder-smb-core`, pure-Rust SMB2/3 + NTLM). Phase 6 (SFTP + SCP) behind `files_ssh_sync` (`russh` 0.45 + `russh-sftp` 2; same dep tree the planned Resource Connect SSH window reuses). Phase 7 (periodic re-sync + on-write inline push) — internal scheduler (single tokio task started from `Core::post_unseal`, ticks every 60s, walks every `files`-typed mount) PLUS external-tick endpoint (`POST /v1/<mount>/sync-tick`) so operators who prefer external scheduling can disable the internal scheduler and drive sweeps from `cron`. Phase 8 (asset-group / cloud / GUI integration). Cluster coordination via `hiqlite::dlock` deferred as a single cross-cutting follow-up alongside the same gap in `pki/auto-tidy` and `ldap/auto-rotate`; the sync push is idempotent (tmp+rename) so a double-push under HA is wasteful but not incorrect.
...
**Phases 1 + 2 + 3 + 4 + 8 shipped** (GUI carries Files page + per-file Info / Sharing / Sync / Versions tabs + Edit modal + drag-and-drop upload with Windows/WebView2 drop-handler fix; resource-detail pages grew a Files tab listing files associated with each resource; Admin → Audit aggregates file lifecycle events). Asset-group membership for files is wired through a third reverse index in `ResourceGroupStore`. Ownership / sharing / admin transfer / backfill flow through the shared `OwnerStore` + `ShareStore` used by KV and Resources. The Sharing tab (`gui/src/components/FileSharingCard.tsx`) grants and revokes against the file's id, and the list page falls back to `identity/sharing/for-me` when `list_files` is denied — a file share covers `files/files/<id>` and its sub-endpoints, never the id-less list path.
...
**Phase 7 (periodic re-sync + sync-on-write) shipped** (`src/modules/files/scheduler.rs`). Two new fields on `FileSyncTarget` let an operator opt into automatic pushes on top of the existing manual `POST /sync/<name>/push`: `auto_sync_interval_seconds` (the scheduler runs a push when the cadence window has elapsed AND the exponential-backoff window has cleared); `sync_on_write` (the file content write handler runs an inline push as part of the same request, returning per-target outcomes in a `sync_on_write[]` array on the response). Three new state fields on `FileSyncState` drive the scheduler decisions (`last_attempt_at_unix`, `next_retry_at_unix`, `consecutive_failures`, `last_attempt_source`). Per-mount `FilesSyncConfig` record at `sync/config` with `enabled` master switch + `max_concurrent_pushes` cap; missing record uses defaults. Manual `POST /v1/<mount>/sync-tick` endpoint runs the same sweep on demand for operators who prefer external scheduling. The manual `/sync/<name>/push`, the inline `sync_on_write`, and the periodic scheduler all funnel through the same `FilesBackendInner::run_sync_push` method — same atomicity, same audit footprint, same backoff. 8 new scheduler unit tests + 2 new integration tests cover the state machine and end-to-end on-write / sync-tick behaviour. **All File Resources phases now shipped.**
