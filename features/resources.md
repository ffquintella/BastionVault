...
- **GUI** (`gui/src/routes/ResourcesPage.tsx`) — list page with type filter + search, detail view with Info / Secrets / History / Sharing tabs, create/edit modal with dynamic type-driven fields, masked secret values with reveal-on-demand, version browser, ownership + admin-transfer modal. Right-clicking a resource card opens a context menu (Open / Connect / **Clone** / Delete); Clone duplicates the resource record under `<name> copy` (deduped, upsert-safe) — secrets/files are not copied.
