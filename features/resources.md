...
- **Server module** (`src/modules/resource/mod.rs`) — metadata CRUD, per-secret CRUD, append-only history log (`hist/<name>/<nanos>` — who/when/which fields, no values), per-secret version snapshots (`smeta/` + `sver/`), built-in + custom resource types stored under the system config space, and a **rename** endpoint (`resources/<name>/rename`, Write, body `{new_name}`). Includes module-level tests.
- **Rename (true identity move)** — `handle_resource_rename` migrates everything keyed by the resource name: metadata (`meta/`), history (`hist/`, carried forward + a `rename` entry appended), secrets and their versions (`secret/` `smeta/` `sver/`), asset-group membership (`ResourceGroupStore::rename_resource`), explicit share grants (`ShareStore::rename_target`, logged as a `rename` op), and ownership (`OwnerStore`). Ordering is write-new-then-delete-old so a mid-failure leaves a recoverable duplicate, never a gap. Attached **file resources** live in a separate mount and are re-pointed via the files engine's `files/repoint-resource` endpoint. The rename refuses a name collision (case-insensitive) and a path-escaping / empty / no-op name. Policy documents are intentionally not rewritten — the GUI warns the operator to update stanzas that reference the old path.
- **Tauri command surface** (`gui/src-tauri/src/commands/resources.rs`) — 15 commands: `list_resources`, `read_resource`, `write_resource`, `delete_resource`, `rename_resource`, `list_resource_secrets`, `read_resource_secret`, `write_resource_secret`, `delete_resource_secret`, `list_resource_history`, `list_resource_secret_versions`, `read_resource_secret_version`, `resource_types_read`, `resource_types_write`, plus ownership/transfer wiring. `rename_resource` orchestrates the resource-engine rename then the files re-point.
- **GUI** (`gui/src/routes/ResourcesPage.tsx`) — list page with type filter + search, detail view with Info / Secrets / Files / Connection / Sharing / History tabs, create/edit modal with dynamic type-driven fields, masked secret values with reveal-on-demand, version browser, ownership + admin-transfer modal. The Info tab's actions are Edit / **Rename** / Delete; Rename opens a small modal that validates the new name, probes for collisions, and warns that the policy path changes. Right-clicking a resource card opens a context menu (Open / Connect / **Clone** / Delete); Clone duplicates the resource record under `<name> copy` (deduped, upsert-safe) — secrets/files are not copied.
