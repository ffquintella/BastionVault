...
- **GUI** — exposed as a "Scheduled backups" tab inside the [`/exchange`](../gui/src/routes/ExchangePage.tsx) page with full CRUD, per-schedule run history, and an immediate "Run now" trigger. The schedule editor offers a **"Back up everything (full vault)"** toggle that saves `scope.kind = "full"`.
- **Tauri commands + `api.ts`** — `scheduledExportsList/Create/Update/Delete/Runs/RunNow` wrappers. These now branch on `VaultMode`: embedded mode dispatches to the in-process `ScheduleStore`; **remote mode routes through the `bv_client::Backend` HTTP API** (`/v{1,2}/sys/scheduled-exports/*`). Before this, the commands were embedded-only and returned a spurious "Vault not open" whenever the desktop client was connected to a remote server.
