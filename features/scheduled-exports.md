...
- **Known limitation — on a cluster the backup set is scattered across nodes.** Deferred leader gating plus Raft-replicated run records interact badly: every node runs its own tick loop, but they all resume from the *same* persisted run record, so the first node to reach a cron instant writes the file and records the run, and the others then see nothing due. The nightly backup therefore lands on whichever node wins that race — a different node from night to night — while `GET /v1/sys/scheduled-exports/{id}/backups` only ever scans the filesystem of the node answering the request. Confirmed in the field on a 2-node cluster (`segdc1vhm0003`/`0004`, 2026-07-27): the 03:00 catch-up fired on node 2 at 16:34:07Z and wrote its file there, node 1 started 27s later and found nothing due, and a listing against node 1 returned an empty `files` array while node 2's directory held the backups. No node has a complete history, and the GUI list appears to lose backups whenever the operator reconnects to a different node. Until leader gating lands, point every schedule at shared storage (an NFS/CSI mount visible to all nodes) so all nodes read and write one directory.
