...
- **Backend** — `src/scheduled_exports/{mod,schedule,store,runner}.rs` (~600 LOC). `schedule.rs` defines the cron-expressed schedule entries with scope, output target, retention policy, and a sealed password reference. `store.rs` is the persisted backing store. `runner.rs` is the tokio-driven tick loop (single-process; leader gating is still deferred, so on a cluster each node fires each schedule against its own destination).
- **Timezone + catch-up (fixed in 0.36.5)** — cron is evaluated in the **server's local timezone**, matching what this spec and the GUI editor promise; it was previously resolved in UTC, so `0 0 3 * * *` fired at 03:00 UTC. First sighting of a schedule after process start resumes from that schedule's most recent persisted run record instead of from "now", so a restart between two cron instants no longer skips the window; catch-up is `single` (one missed instance, never a replay of every missed instance, with a bounded per-tick scan). A schedule that has never run still resumes from "now".
- **Startup dependency (fixed in 0.36.5)** — the tick loop is spawned from `Core::post_unseal`. On HSM auto-unsealing servers that call ran on a throwaway bootstrap runtime that was dropped right after, so the spawned loop — along with every other `post_unseal` background worker — was discarded before its first poll and no schedule ever fired (no run records, no audit events, no logs). Auto-unseal now runs on the Actix system runtime (`src/cli/command/server.rs`). `scheduled-exports: scheduler started` in `operations.log` is the signal that the loop is alive; each firing logs `scheduled-exports: firing schedule <id> (<name>) for cron instant <local time>`.
- **Destination persistence (fixed, unreleased)** — the destination is a directory on the *server's* filesystem, and the official container image declared a volume only for `/var/lib/bvault/data`. A schedule pointed at any other path (e.g. `/backups`) wrote into the container's ephemeral writable layer, so the files were discarded whenever the container was recreated while the schedule itself survived on the data volume — a schedule that keeps firing with an empty **Backups** list and nothing in the logs, because each write really did succeed. The image now pre-creates and declares `/var/lib/bvault/backups` owned by UID 65532, both reference compose files mount it (per-node in `cluster.yml`, since leader gating is still deferred), `deploy/container/README.md` documents it, and the GUI now says the path resolves on the vault host and must be persistent — with the Backups modal warning explicitly when successful runs are on record but the directory scan comes back empty. Each successful run also logs the absolute file it wrote (`scheduled-exports: wrote <path> (<n> bytes)`), since the run record only carries the destination directory and a byte count.
- **Cluster-wide backup catalog + cross-node restore (added, unreleased).** Deferred leader gating plus Raft-replicated run records mean the nightly file lands on whichever node reaches the cron instant first — a different node from night to night — and a plain `read_dir` of the destination is therefore a per-node view. Observed in the field on a 2-node cluster (`segdc1vhm0003`/`0004`, 2026-07-27): the 03:00 catch-up fired on node 2 at 16:34:07Z and wrote its file there, node 1 started 27s later and found nothing due, and a listing against node 1 returned an empty `files` array while node 2's directory held the backups — which reads to an operator as "the backups did not survive the restart".

  The fix replicates the *metadata* instead of requiring shared storage:

  - `src/scheduled_exports/catalog.rs` records every produced file at `core/scheduled_exports/backups/<schedule_id>/<filename>` — size, format, SHA-256, write time, and the owning node's Raft id / host name / `api_addr`. Barrier-encrypted, so it replicates with everything else. A catalog write failure never fails an otherwise-good backup.
  - `GET /v1/sys/scheduled-exports/{id}/backups` merges the catalog with the answering node's directory scan, so **every** node lists **every** backup, each row annotated with `node_id` / `node_name` / `api_addr` plus `local` (restorable here without a fetch) and `present` (false for our own record whose file has vanished). Files with no record — pre-catalog runs, manual copies — are still listed from the scan.
  - `POST /v1/sys/scheduled-exports/{id}/restore` transparently fetches a file it does not hold from the owning node via the new `GET /v1/sys/scheduled-exports/{id}/backups/{filename}/fetch` (base64 body), carrying **the caller's own token** — no standing cluster credential — and verifies the fetched bytes against the catalog's SHA-256 before decrypting. The fetch endpoint is local-only and never forwards, so a stale record cannot create a loop. Peer TLS verifies against the platform trust store, or against the listener's `tls_publish_ca_path` when set (private-CA / self-signed clusters).
  - The node must advertise `api_addr` in its config to be fetchable; when it does not, the listing still shows where the file is and the restore fails with a message naming the node instead of a bare "not found".
  - Deleting a schedule purges its catalog records (`ScheduleStore::delete`) but never the files.

  Shared storage (NFS/CSI at one path on every node) remains a valid deployment choice and needs no catalog, but is no longer required to see or restore a cluster's backups.
...
- **GUI** — exposed as a "Scheduled backups" tab inside the [`/exchange`](../gui/src/routes/ExchangePage.tsx) page with full CRUD, per-schedule run history, and an immediate "Run now" trigger. The schedule editor offers a **"Back up everything (full vault)"** toggle that saves `scope.kind = "full"`. Each schedule card also has a **Backups** button that opens a modal listing the backup files the runner has written to that schedule's destination directory (name, format, size, modified, newest first), each with a **Restore** button offering a classified preview (new/identical/conflict) then an apply under a selectable conflict policy (skip/overwrite/rename), prompting for the `.bvx` password when needed.
- **Tauri commands + `api.ts`** — `scheduledExportsList/Create/Update/Delete/Runs/RunNow` wrappers, plus `scheduledExportsBackupsList` (backup listing) and `scheduledExportsRestore` (unified preview/apply restore). All branch on `VaultMode`: embedded mode reads/imports the on-disk file in-process via `ScheduleStore` + the Exchange `import_from_document` path; **remote mode routes through the `bv_client::Backend` HTTP API** (`/v{1,2}/sys/scheduled-exports/*`, including the `backups` and `restore` endpoints) so the file is enumerated, read, and imported on the server. Before this, the backup-listing and restore commands were embedded-only and failed fast with *"... only available when running an embedded vault"* when the desktop client was connected to a remote server.
- **Full-vault scope** — `ScopeKind::Full` is honoured at the `exchange::scope::export_to_document` chokepoint, so cron runs, "Run now", and the one-shot exchange export all enumerate every KV mount, resource, file blob, and group when the schedule is `full`.
...
GET    /v1/sys/scheduled-exports/<id>/backups                 # list backup files in the destination dir
POST   /v1/sys/scheduled-exports/<id>/restore                 # read one file off the server disk + import (dry_run to preview)
```

The `backups` + `restore` endpoints exist so the GUI's Scheduled-backups tab works against a **remote** vault: the files live on the server's disk, so both enumeration and the restore read/import run server-side. `restore` takes `{ filename, password?, allow_plaintext?, conflict_policy?, dry_run? }`; `dry_run: true` classifies (new/identical/conflict) without writing, otherwise it applies via the shared `import_from_document` path.
...
- ~~**In-process restore from a `.bvx` file the scheduler produced.**~~ *(Now in scope.)* Restore reuses the Exchange `import_from_document` machinery (classified preview → apply under a conflict policy), surfaced as the `/sys/scheduled-exports/<id>/restore` endpoint so a remote GUI can read + import a server-side backup file without round-tripping the bytes through the client.
