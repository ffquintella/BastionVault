...
- **GUI** — exposed as a "Scheduled backups" tab inside the [`/exchange`](../gui/src/routes/ExchangePage.tsx) page with full CRUD, per-schedule run history, and an immediate "Run now" trigger. The schedule editor offers a **"Back up everything (full vault)"** toggle that saves `scope.kind = "full"`. Each schedule card also has a **Backups** button that opens a modal listing the backup files the runner has written to that schedule's local destination directory (name, format, size, modified, newest first), each with a **Restore** button that drives the standard Exchange import preview→apply flow on the on-disk file (password + conflict policy) — no re-upload, no separate restore endpoint.
- **Tauri commands + `api.ts`** — `scheduledExportsList/Create/Update/Delete/Runs/RunNow` wrappers, plus `scheduledExportsBackupsList` / `scheduledExportsBackupRead` for the on-disk backup listing + restore (embedded-only; they touch the host filesystem and fail fast in remote mode). These now branch on `VaultMode`: embedded mode dispatches to the in-process `ScheduleStore`; **remote mode routes through the `bv_client::Backend` HTTP API** (`/v{1,2}/sys/scheduled-exports/*`). Before this, the commands were embedded-only and returned a spurious "Vault not open" whenever the desktop client was connected to a remote server.
