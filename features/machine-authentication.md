...
- Phase 5 (client CLI) lands on a parallel branch; Phase 7 (direct-SVID + CRL enforcement + hardening/docs)
  is not started. (`sync_handler` build of `cmis_grpc` is unsupported — async-only — and that build config is
  independently broken repo-wide. Audit events are log-only; no dedicated audit-store rows yet.)
