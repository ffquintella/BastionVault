...
1. **JSON schema (`bvx.v1`)** — a stable, versioned, human-readable JSON document describing a selected subset of vault data: KV items, resources, file-resource metadata + payload (base64), asset / resource-group memberships, opaque per-engine subtrees, the ACL policies of a whole-vault scope, and (opt-in) identity entities.
...
- **CLI** — `src/cli/command/{exchange,exchange_export,exchange_import,exchange_verify}.rs` provide `bvault exchange export` / `preview` / `import` / `verify` with `--password-stdin`. `export` takes either `--scope` selectors or one of the whole-vault modes — `--full` (the targeted namespace) or `--all-namespaces` (every namespace, bundled) — which are mutually exclusive with each other and with `--scope`; a selective export with no selectors is refused before the password prompt. The `--password=...` flag is intentionally refused. `verify` is fully offline (no vault/network): it decrypts + structurally validates a `.bvx`/JSON file, re-hashes every embedded file blob against its recorded `sha256`/`size_bytes`, and fails on an empty document. Library entry point `exchange::verify_backup_bytes` → `VerifyReport` (`src/exchange/verify.rs`).
- **GUI** — `gui/src/routes/ExchangePage.tsx` (~960 lines) ships Export + Import tabs with the scope picker, the password input + entropy meter, the preview classification table with per-item / bulk-set conflict policies, and the final Apply step with the summary panel. The backing Tauri commands (`gui/src-tauri/src/commands/exchange.rs`: `exchange_export` / `exchange_preview` / `exchange_apply`) branch on `VaultMode`: embedded mode dispatches in-process against the open vault, **remote mode routes through the `bv_client::Backend` HTTP API** (`/v1/sys/exchange/{export,import/preview,import/apply}`). Because the preview token is server-side (`core.exchange_preview_store`), remote preview *and* apply both hit the server. Before this, the three commands locked `state.vault` directly and failed with *"Vault not open"* whenever the desktop client was connected to a remote server.
- **Full export from the GUI** — the Export tab's *What to export* selector offers **Full vault (everything you can read)** alongside the selector list. Picking it sends `scope.kind = "full"` with an empty `include` (`api.exchangeExport(..., scopeKind)` → the `scope_kind` argument on the `exchange_export` Tauri command → `parse_scope`), which is what scheduled backups already used but the interactive page could not reach — the command hardcoded `Selective`, so a full export was CLI/scheduler-only. Full mode hides the scope picker, pins the format to password-encrypted `.bvx` (the plaintext-JSON escape hatch stays selective-only), downloads as `vault-full-export.bvx`, and requires an extra confirm before running. `scope_kind` is recorded in the export's audit entry. Covered by `gui/src/test/exchangeFullExport.test.tsx`.
- **All-namespaces export** — `scope.kind = "all_namespaces"` (`src/exchange/namespaces.rs`) exports the **whole deployment**, not just the namespace the request targets. Every other scope kind resolves against a single `MountIndex`, and the root index only addresses `namespaces/<root_uuid>/…` — so a `full` export from the root captured nothing from any child namespace. `export_all_namespaces` builds a `MountIndex` per namespace from that namespace's own `MountsRouter` (`MountIndex::from_namespace_router`, prefixes `namespaces/<uuid>/{logical,sys}/`), runs the same `resolve_scope` sweep for each, leaves the root's items at the document's top level, and appends one `NamespaceBundle { path, items }` per non-root namespace (`ExchangeItems.namespaces`, sorted by path, empty bundles dropped). Bundles are what stop two tenants' identically-named `secret/` mounts from colliding into one flat item list. The field is `#[serde(default, skip_serializing_if = "Vec::is_empty")]`, so an `all_namespaces` document is a strict superset of a `full` one and older single-namespace documents parse unchanged. Import is namespace-aware to match — `exchange::import_all_namespaces` writes each bundle through its namespace's index and reports a bundle whose namespace is missing on the destination as an `ImportResult::warnings` entry (shown in the GUI Preview panel) instead of writing tenant data into root; the root-only `scope::import_from_document` warns rather than misplacing bundles. Reachable from the Export tab (root sessions only — a child-namespace session may only export itself), `POST /v1/sys/exchange/export`, and scheduled backups. **Data plane plus each namespace's ACL policies** (see the policies bullet below): the namespace registry, mount tables, and identities remain control plane — use the operator BVBK backup, which is a raw physical-backend sweep and therefore already spans every namespace (pinned by `backup_captures_every_namespace` in `src/backup/create.rs`).
- **ACL policies travel with a whole-vault export** — a `full` / `all_namespaces` export used to capture the data plane only, so a restore rebuilt every secret, engine, resource, and file and then left the vault unusable by anything but a root token: the policies that authorize access to that data were never in the file. `resolve_full` now also sweeps the exporting namespace's ACL policy keyspace into `ExchangeItems.policies` (`PolicyItem { name, value, tests? }` — the stored policy document plus the policy's saved effectivity test cases when it has any), and `apply_items` writes them back through `MountIndex::policy_prefix()` / `policy_tests_prefix()`. Namespace-correctness comes from the document's shape, exactly as it does for KV: a tenant's policies ride in that tenant's `NamespaceBundle`, never at the top level. That matters because tenant policies do **not** live under the tenant's own `sys/` — the policy store keeps them at `policy-ns/<b64url(path)>/acl/` inside the *root* system view — so `MountIndex` carries the root system prefix and the namespace path, and the layout itself is owned by one pair of functions in the policy store (`acl_keyspace` / `policy_tests_keyspace`) that the live store, the exporter, and the importer all call. Selective exports are deliberately unchanged (see the `optional` decision below): sharing a handful of secrets must not ship authorization rules. Vault-owned policies (`IMMUTABLE_POLICIES`: `root`, `response-wrapping`, `control-group`) are excluded from exports and refused on import; a name that could escape the keyspace (`../../core/mounts`) is refused with an `ImportResult::warnings` entry rather than written. A non-dry-run import that touched policies flushes the policy store's LRU — a cached pre-import document would otherwise keep serving authorization decisions the operator had already replaced. `ItemCounts.policies` (and the previously-missing `raw`) are reported by `bvault exchange verify`. Pinned by `policies_full_capture_and_restore` + `policy_import_refuses_unsafe_and_reserved_names` (`src/exchange/scope.rs`) and `all_namespaces_export_round_trips_policies` (`src/exchange/namespaces.rs`).
- **Version-skew legibility** — the export endpoint reports *why* it refused a body instead of collapsing every case into `RvError::ErrRequestInvalid` (`Request is invalid.`). A client naming a scope the server's build does not know — a 0.37.x GUI asking a pre-0.37.0 server for `all_namespaces` — is the common case: `ScopeKind` is a plain serde enum, so one unknown value rejects the entire body, and the operator saw a bare `HTTP 400: Request is invalid.` on a page whose only changed field was the scope. The handler now returns `invalid export request body: <serde error>` (which names the unknown variant) and `password required for format "bvx"` for the other 400 it could produce; the GUI's `isUnsupportedRequestShape` (`gui/src/lib/error.ts`) recognises both that shape and the old generic wording, and the Export tab / scheduled-backup editor append "the connected server does not support the all-namespaces scope (added in 0.37.0)" when the refused scope was whole-deployment. Pinned by `exchange_export_route_tests` in `src/http/sys.rs` (unknown variant, missing password, and `all_namespaces` accepted end-to-end over HTTP) and by `gui/src/test/{error,exchangeFullExport}.test.*`. The import paths deliberately keep the generic error: their body carries the document itself, and a serde message can quote payload values.
...
- **Full-vault engine coverage** — a `kind: "full"` export (the scheduled-backup default) now captures **every** secret engine, not just the structurally-modelled KV / resource / files / group types. `resolve_full` sweeps each remaining logical mount (PKI, SSH, SSH-broker, Transit, TOTP, OpenLDAP, Rustion, and any future engine) and records its barrier subtree verbatim as opaque `RawEntry` items (`ExchangeItems.raw`); `system`/`identity` are skipped (`RAW_SKIP_TYPES`) so the live mount table / seal config never round-trips. `import_from_document` restores those raw entries **and** the structured resources / files / groups that earlier versions exported but silently dropped on restore, all through one `dry_run`-aware classify+write path (`apply_entry`) shared by the real write and the *Preview*. The preview classification, previously reimplemented KV-only in four places, now routes through that engine so it agrees with the write across all item types.
...
```json
{
  ...
  "items": {
    ...
    "resource_groups": [ ... ],
    "raw": [
      { "mount": "pki/", "path": "certs/1a2b", "value": {...} }
    ],
    "policies": [
      {
        "name": "reader",
        "value": { "version": 2, "raw": "path \"secret/*\" {...}", "templated": false, "type": "acl" },
        "tests": [ { "path": "secret/data/x", "capability": "read", "expect": "allow" } ]
      }
    ],
    "namespaces": [
      { "path": "engineering", "items": { "kv": [...], "policies": [...] } }
    ]
  },
  "optional": {
    "entities":  [ ... ],
...
- **Policies follow the scope, not a flag.** The original design put policies behind an opt-in `optional.policies` / `--include-policies`. That is the right call for a *share* and the wrong one for a *backup*: a whole-vault file without the policies is not restorable into a working vault. So `full` / `all_namespaces` exports always carry `items.policies` (each namespace's in its own item set), and `selective` exports never do — the over-scope concern the flag existed for only applies to sharing a handful of secrets. Identity (`optional.entities` / `groups`) remains unimplemented and out of scope; use the operator BVBK backup for the rest of the control plane.
