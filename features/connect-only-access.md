...
| Session-open gate | Both open routes (`handle_session_open`, `handle_session_open_v2`, `src/modules/rustion/mod.rs`) run a **secondary ACL check** via `may_connect_resource` before resolving any credential. Distinct from the `Write` gate `post_auth` runs on the endpoint path itself. Two ways to pass: an **explicit ungated** `connect`/`read` grant on `<ns>/resources/secrets/<name>/` (probed with `ACL::explain_capability`, which fails scope-gated rules closed so the share-scoped baseline can't read as a blanket grant — this is the connect-only shape), **or** ownership-/share-derived access (probed with `PolicyStore::readable_targets`, which hydrates the owner / share / asset-group qualifiers the real pipeline resolves). The second was missing, so every share-grantee and most resource owners were refused and fell back to the direct dial. |
...
| GUI capability lookup | `v2/sys/capabilities-self` (`handle_capabilities_self`, `src/modules/system/mod.rs`; HTTP shim `/v2/sys/capabilities-self`, v2-only) returns the caller's effective capabilities per path. Vault-compatible shape. Clients send **mount-relative** paths (`resources/secrets/<name>/`); the handler namespace-qualifies them before evaluating (`qualify_capability_path`) so the verdict matches what the router will decide, and keys the response by the path the caller sent. Header-scoped mounts (`sys/` `auth/` `identity/` `rustion/`) are exempt, exactly as in the router. |
