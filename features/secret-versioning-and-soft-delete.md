...
- **Errors** — `crates/bv-errors/src/lib.rs` carries `ErrModuleKvV2CasMismatch`, `ErrModuleKvV2CasRequired`, `ErrModuleKvV2VersionDestroyed`, `ErrModuleKvV2VersionNotFound`, `ErrModuleKvV2DataFieldMissing` with appropriate HTTP status mapping.
...
- **Version selector transport** — a versioned read carries the version as a `?version=<n>` query parameter, never a request body. `bv-server::logical_routes` parses a JSON body for POST/PUT only and lifts just the allowlisted `env`/`version` query keys into `Request::data`, so a body-borne version on a `GET` is dropped and `handle_data_read` silently answers with `current_version`. `get_version_from_request` reads `req.data` before `req.body` so both the remote and embedded backends behave identically.
- **Tests** — `tests/test_default_logical.rs` carries `test_kv_v2_logical_backend` (full lifecycle, soft-delete, undelete, destroy, CAS, max_versions, metadata list, delete-all, config persistence) and `test_kv_v2_version_history_tracking` (username + operation propagation). `src/engine_tests/kv_version_read.rs` pins the version-selector transport over a real HTTP server — an in-process `core.handle_request` test cannot observe that class of bug.
