...
- **Async bootstrap auto-wired through `storage::new_backend_async`** — both the desktop (Tauri) and the server-mode (`bvault server`) bootstraps route through this constructor, which dispatches to `FileBackend::new_maybe_obfuscated` for the `file` backend. The sync `FileBackend::new` warning now fires only on `operator backup` / `restore` / `migrate`, which intentionally see the underlying hashed keys.
- **Plaintext-key manifest** at `_bvault_manifest` (newline-delimited list, maintained on every write/delete) makes salt rotation possible without inverting the HMAC. One extra small read+write per vault op; failures log a warning but do not roll back the data write.
- **`bvault operator cloud-target rekey-salt` CLI** — orchestrates in-place salt rotation. Operator seals the vault, runs the CLI, unseals; the CLI walks the manifest, re-writes each entry from `HMAC(old_salt, key)` to `HMAC(new_salt, key)`, atomically swaps `_bvault_salt`, and cleans up orphan blobs. Crash-safe: an interruption between the re-write loop and the salt swap leaves the vault bootable against the old layout. `--dry-run` reports the manifest size without touching data.
