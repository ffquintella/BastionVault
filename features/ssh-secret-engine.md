...
- **Issuance audit trail**: every successful `ssh/sign/:role` (classical or PQC) appends to `SshSignAuditStore` ([`src/modules/ssh/ssh_sign_audit_store.rs`](../src/modules/ssh/ssh_sign_audit_store.rs)) and surfaces on the admin Audit page under the **SSH Sign** category — recording actor, role, principals, cert type, the 16-hex-digit serial (the same `serial_number` returned to the caller, for revocation/forensics), and the signing algorithm. Fail-soft like the CA audit store: a recording failure logs at WARN and never blocks the signing. The cert body itself is never stored — only this metadata.
