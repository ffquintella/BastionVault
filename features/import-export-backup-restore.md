...
- **Desktop GUI (File -> Backup)**: `gui/src-tauri/src/commands/backup.rs` wraps
  ...
  Both directions call `require_root`, which refuses any token lacking the
  literal `root` policy. Because auth backends cannot mint root tokens, the
  menu items are only shown to root-token sessions --
  `gui/src/components/Layout.tsx` withholds the handlers otherwise and
  `AppMenu` drops the submenu.
