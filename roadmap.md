...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–5 + 3.1 + 3.2 + 4.1 + 4.2 light + 6.1 + 6.2 + 6.3 done** (BV 0.7.14 → 0.7.26, Rustion 0.7.11 → 0.7.22): target registry + health, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/{open,renew,kill}` + `POST rustion/recordings/pull`, GUI ConnectionProfile `kind` + `useRustionSessionLifecycle` auto-renew hook + `rustionRecordings*` API surface, SSH + RDP ticket-auth, SSH + RDP listener-loop wire-up with authority/correlation_id stamping, NTLMv2 primitives + CredSSP scaffold validated against MS-NLMP §4.2.4 vectors (Phase 4.2 light), recording sidecar JSON with sha256 (Phase 6.1), hybrid-signed `recording.ready` webhook with `deliver_with_retry` 5×exp-backoff + BV receiver + recordings index + 24h pull-fallback endpoint and helper (Phases 6.2 + 6.3), e2e docker-compose stack. Phase 4.2-full (RC4 sealing + pubKeyAuth + Windows VM), Phase 6.4 (24h cron scheduler + GUI playback + signed-URL bytes endpoint), and Phase 7 (policy tiers) next. |
