...
| Resource Connect — in-app SSH / RDP for server resources ([spec](features/resource-connect.md)) | Partial — Phases 1 + 2 + 3 done. Phase 1: structured `os_type` select + `select` field type. Phase 2: Connection profiles + Connection tab + Secret source editor. Phase 3: **Connect button live for SSH+Secret profiles** — new Tauri WebviewWindow per session (xterm.js + `russh` 0.45), four `session_*` Tauri commands (`open_ssh` / `input` / `resize` / `close`), credential bytes pulled from the resource's barrier-encrypted secret directly into russh (never reach the JS layer or the OS clipboard), TOFU host-key handling with optional `host_key_pin`, 30 s connect+auth timeout, window-close hook tears down the SSH session as a side-effect. Phases 4 (RDP via `ironrdp`), 5 (LDAP source), 6 (PKI source), 7 (polish + per-type policy) follow. |
