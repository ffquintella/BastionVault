...
| Resource Connect — in-app SSH / RDP for server resources ([spec](features/resource-connect.md)) | Partial — Phases 1 + 2 + 3 + 4 + 5 + 6 done. Phase 1: structured `os_type` + `select` field type. Phase 2: Connection profiles + Connection tab + Secret source editor. Phase 3: **SSH+Secret** Connect (xterm.js + `russh`). Phase 4: **RDP+Secret** Connect (`<canvas>` + `ironrdp` 0.14 from a [`IronRDP/`](IronRDP/) submodule on `fix-deps`). Phase 5: **LDAP credential source** for both SSH and RDP — `operator` / `static_role` / `library_set` bind modes; library check-in fires automatically on session-close. Phase 6: **PKI credential source** — at connect the host calls `pki/issue/<role>` with the resource hostname as CN and feeds the issued `private_key` to russh as a publickey credential (SSH only this cut). RDP+PKI requires CredSSP smartcard wiring (sspi-rs `scard` backend) — scaffolded but stubbed at the resolver. Launch matrix: SSH × {Secret, LDAP, PKI} ✅, RDP × {Secret, LDAP} ✅, RDP × PKI / SSH-engine = pending. Phase 7 (polish + per-type policy + recently-connected) follows. |
