...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–8 + 9.1 done, Phase 4.2-full done** (BV 0.7.14 → 0.7.37, Rustion 0.7.11 → 0.7.28): full session/recording pipeline + four-tier policy + telemetry pull + audit-witness replication + rate-limited telemetry + recording.replayed audit + fleet analytics + stable deployment_id + Rustion authorities-pending holding pen + approval/reject workflow + tombstones + deployment_id binding (403 attestation_mismatch) + separate SessionReplayWindow Tauri WebviewWindow + WASM bitmap-update decoder at `gui/wasm/rdp-replay/` with matching TS port + RdpReplayCanvas live playback (uncompressed 16/24/32 bpp + RLE16/RLE24) + signed-URL replay path (`POST /v1/recordings/<rid>/replay` + HMAC-bound `GET /v1/recordings/<rid>`) + bastion-driven CredSSP injection driver (NTLMv2 sealing + RC4 keystream + pubKeyAuth +1 contract + sealed TSCredentials, simulated-Windows e2e tested). Phase 9.2 (disk-backed pending/tombstones + approval CLI + admin UI + deenrol + weekly re-attestation timer) and live-Windows-VM transport hookup remaining. NSCodec / RemoteFX / 8 bpp RLE / bitmap-cache references and `rdp-cert` smart-card PKINIT documented as separate tracks. |
