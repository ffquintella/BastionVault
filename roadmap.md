...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phase 1 done in 0.7.14**: target registry, three-strikes health state machine, live 30s HTTP pinger, master-cert config slot, audit event taxonomy, CLI subcommands, GUI Settings → Rustion Bastions section. Phase 2 (BVRG-v1 envelope crate + master cert lifecycle) next. |
