...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. Phase 1 scaffold landed in 0.7.12 (target registry, health state machine with 3-strikes debounce, master-cert config slot, audit event taxonomy, HTTP routes). GUI / CLI / live probe pending. |
