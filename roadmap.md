...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–4 done** (BV 0.7.14 → 0.7.18, Rustion 0.7.11 → 0.7.14): target registry + health probing, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/open` end-to-end, GUI ConnectionProfile `kind` discriminator, SSH + RDP ticket-auth modules. Phases 3.1 + 4.1 (russh + ironrdp listener-loop wire-up + e2e demo against real targets) follow as their own slice. Phase 5 (renew/terminate) next. |
