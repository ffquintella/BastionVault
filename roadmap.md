...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–4 + 3.1 + 3.2 done** (BV 0.7.14 → 0.7.20, Rustion 0.7.11 → 0.7.16): target registry + health, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/open`, GUI ConnectionProfile `kind`, SSH + RDP ticket-auth modules, SSH listener-loop wire-up with all three credential kinds (password / private-key / OpenSSH-cert), recording authority/correlation_id stamping, e2e docker-compose stack with Dockerfiles in both repos. Phase 4.1 (ironrdp listener wire-up) follows. Phase 5 (renew/terminate) next. |
