...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–3 done** (BV 0.7.14 → 0.7.17, Rustion 0.7.11 → 0.7.13): target registry + health probing, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher with pinned-list and random-pool modes, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/open` driving the full dispatcher → envelope → POST → ticket pipeline, GUI ConnectionProfile `kind` discriminator + bastions list, ticket-auth module on Rustion side. Phase 3.1 wires the russh listener into ticket_auth + adds the e2e demo; Phase 4 (RDP) and Phase 5 (renew/terminate) follow. |
