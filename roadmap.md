...
| `[ ]` Todo | Plugin App Extensions (Extensibility v2: dynamic menus, plugin windows, vault-API + admin-granted network from sandboxed app modules) | [spec](features/plugin-app-extensions.md), [roadmap](roadmaps/plugin-app-extensions.md) — spec ratified. Tauri-backend Wasmtime app modules with capability-gated `bvx.*` host imports; network double-gated by manifest flag + install-time admin grant (`/v1/sys/plugins/<name>/grants`, audited, capability-hash-pinned, SSRF-guarded). 8 phases, ≈11 engineer-weeks. |
