...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–8 + 9.1 done** (BV 0.7.14 → 0.7.35, Rustion 0.7.11 → 0.7.27): full session/recording pipeline + four-tier policy + telemetry pull + audit-witness replication + rate-limited telemetry + recording.replayed audit + fleet analytics + stable deployment_id + Rustion authorities-pending holding pen + approval/reject workflow + tombstones + deployment_id binding (403 attestation_mismatch) + separate SessionReplayWindow Tauri WebviewWindow + WASM frame-walker crate at `gui/wasm/rdp-replay/` + signed-URL replay path (`POST /v1/recordings/<rid>/replay` + HMAC-bound `GET /v1/recordings/<rid>`). Phase 4.2-full (RC4 sealing + Windows VM), Phase 8.4 (RDP bitmap-update visual codec — separate codec engineering track), Phase 9.2 (disk-backed pending/tombstones + approval CLI + admin UI + deenrol + weekly re-attestation timer) remaining. |
