...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–7 + 8.1 + 8.2 done** (BV 0.7.14 → 0.7.33, Rustion 0.7.11 → 0.7.25): full session/recording pipeline + four-tier policy + telemetry pull + audit-witness replication + per-(IP,authority) rate limiting + `recording.replayed` audit event with sha256 integrity check + fleet analytics (top targets / top operators) on the Live Sessions page. Phase 4.2-full (RC4 sealing + pubKeyAuth + Windows VM), Phase 8.3 (replay WebviewWindow + `.rdp-rec` WASM decoder + signed-URL replay infrastructure), Phase 9 (enrolment lifecycle) next. |
