...
| Resource Connect — in-app SSH / RDP for server resources ([spec](features/resource-connect.md)) | Partial — Phases 1 + 2 + 3 + 4 done. Phase 1: structured `os_type` select + `select` field type. Phase 2: Connection profiles + Connection tab + Secret source editor. Phase 3: **SSH+Secret** Connect — Tauri WebviewWindow + xterm.js + `russh`. Phase 4: **RDP+Secret** Connect — Tauri WebviewWindow + `<canvas>` + `ironrdp` 0.14 from a [`IronRDP/`](IronRDP/) submodule on `fix-deps` branch (bumps picky to rc.23 + patches sspi to its main SHA so `crypto-common` resolves to stable 0.2.1 alongside the host's `digest 0.11` stack). RDP Standard Security only this cut (no NLA / CredSSP). Three new Tauri commands (`session_open_rdp`, `session_input_rdp_mouse`, `session_input_rdp_key`); `session_close` fan-outs to either SSH or RDP variant. Phases 5 (LDAP source), 6 (PKI / CredSSP), 7 (polish + per-type policy) follow. |
