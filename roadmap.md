...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–6 done end-to-end** (BV 0.7.14 → 0.7.28, Rustion 0.7.11 → 0.7.23): target registry + health, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/{open,renew,kill}` + `POST rustion/recordings/pull` + `GET rustion/recordings/<rid>/blob`, GUI `/recordings` page with inline xterm.js asciicast playback + `.rdp-rec` frame walker + download, `useRustionSessionLifecycle` auto-renew hook, SSH + RDP ticket-auth, SSH + RDP listener-loop wire-up with authority/correlation_id stamping, NTLMv2 primitives + CredSSP scaffold validated against MS-NLMP §4.2.4 vectors (Phase 4.2 light), full recording handoff pipeline: sidecar JSON with sha256 + hybrid-signed `recording.ready` webhook with 5×exp-backoff retry + BV receiver + recordings index + pull-fallback helper + 24h cron poller + signed bytes-endpoint + proxy emit-glue (Phases 6.1 → 6.5), e2e docker-compose stack. Phase 4.2-full (RC4 sealing + pubKeyAuth + Windows VM), Phase 7 (policy tiers), Phase 8 (telemetry + in-GUI replay), Phase 9 (enrolment-approval + re-attestation + tombstones) next. |
