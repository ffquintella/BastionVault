...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–7 + 8.1 + 8.2 + 9.1 done** (BV 0.7.14 → 0.7.34, Rustion 0.7.11 → 0.7.26): full session/recording pipeline + four-tier policy + telemetry pull + audit-witness replication + rate-limited telemetry endpoints + recording.replayed audit with sha256 integrity check + fleet analytics + stable deployment_id stamped on every envelope + Rustion authorities-pending holding pen + approval/reject workflow + tombstones + deployment_id binding enforcement (403 attestation_mismatch). Phase 4.2-full (RC4 sealing + Windows VM), Phase 8.3 (replay WebviewWindow + `.rdp-rec` WASM decoder + signed-URL replay), Phase 9.2 (disk-backed pending/tombstones + approval CLI + admin UI + deenrol + weekly re-attestation timer) remaining. |
