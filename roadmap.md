...
| `[/]` In progress | Namespaces / Multi-tenancy | [spec](features/namespaces-multitenancy.md) — **Phase 1 foundation landed**: `src/modules/namespace/` with hierarchical namespace registry (path↔UUID, validation, parent/child indices), request→namespace resolver (header + path-prefix forms), per-namespace mount-router registry (reuses `MountsRouter` router-prefix), `v2/sys/namespaces` CRUD on the system backend, and an idempotent **non-destructive** barrier re-root copy (legacy keys stay authoritative; activating the re-rooted prefix is gated behind `BASTION_NAMESPACE_REROOT`, default off). 9 unit/integration tests. Remaining in Phase 1: end-to-end per-namespace mount creation + dispatch. Phases 2–4 (per-namespace policy/token/audit, identity + cross-tenant linking, quotas + GUI) not yet started. |
