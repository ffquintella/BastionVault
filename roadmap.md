...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–7 + 8.1 done end-to-end** (BV 0.7.14 → 0.7.32, Rustion 0.7.11 → 0.7.24): full session/recording pipeline + four-tier policy + telemetry pull. New: Rustion `/v1/sessions/{active,history}` + `/v1/stats` authority-scoped telemetry endpoints, BV 60s telemetry poller with cursor persistence, GUI `/rustion-sessions` Live Sessions page with auto-refresh + per-row Terminate. Phase 4.2-full (RC4 sealing + pubKeyAuth + Windows VM), Phase 8.2 (signed audit witness + replay window + analytics dashboard), Phase 9 (enrolment lifecycle) next. |
