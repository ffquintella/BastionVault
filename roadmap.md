...
| `[ ]` Todo | GUI Dashboard Redesign (operational PAM landing view) | [spec](features/gui-dashboard-redesign.md) — replaces the static mounts/auth listing with KPI tiles + session-activity chart + live sessions + audit feed + needs-attention panel, modelled on CyberArk / Delinea / Vault / Teleport landing views. backend-first (Phase 1 = `GET /v1/sys/dashboard/summary` aggregation endpoint) + 4 GUI phases rendering against it, with `Promise.allSettled` client-side fallback + graceful degradation. |
...
- GUI Dashboard Redesign (operational PAM landing view — KPI tiles + session activity + live sessions + audit feed + attention panel)
