...
| `[/]` Partial | Native Client Installers (deb / rpm / pkg / msi / nupkg for GUI + CLI) | [spec](features/packaging-client-binaries.md) — Tauri bundler for GUI, parallel CLI track via `cargo-deb` / `cargo-generate-rpm` / `pkgbuild` / WiX 3.x. Double-signed (platform-native + Cosign). Phase 5 publishes apt + dnf repos. **CLI local builds shipped**: Linux .deb/.rpm (`make linux-cli-packages`) and Windows .msi/.nupkg (`make windows-cli-packages` — WiX 3.x project + Chocolatey package, unsigned, x64); `make cli-packages` dispatches per host OS. GUI installers, macOS .pkg, CI matrix + signing pending. |
