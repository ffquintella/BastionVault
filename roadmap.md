...
| Resource Connect — in-app SSH / RDP for server resources ([spec](features/resource-connect.md)) | Partial — Phases 1 + 2 + 3 + 4 + 5 done. Phase 1: structured `os_type` + `select` field type. Phase 2: Connection profiles + Connection tab + Secret source editor. Phase 3: **SSH+Secret** Connect (xterm.js + `russh`). Phase 4: **RDP+Secret** Connect (`<canvas>` + `ironrdp` 0.14 from a [`IronRDP/`](IronRDP/) submodule on `fix-deps`). Phase 5: **LDAP credential source** for both SSH and RDP — three bind modes: `operator` (frontend prompt for user/password at connect; never persisted), `static_role` (host reads `<mount>/static-cred/<role>` internally), `library_set` (host calls `library/<set>/check-out`, registers a session-close hook that runs `check-in` automatically). Unified credential resolver returns optional canonical username + optional on-close cleanup hook so engine-issued sources slot in cleanly. Phases 6 (PKI / CredSSP smartcard), 7 (polish + per-type policy + recently-connected) follow. |
