...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–6 + 7.1 + 7.2 done** (BV 0.7.14 → 0.7.30, Rustion 0.7.11 → 0.7.23): target registry + health, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/{open,renew,kill}` + `POST rustion/recordings/pull` + `GET rustion/recordings/<rid>/blob` + four-tier `rustion/policy/*` CRUD + `POST rustion/policy/force-rustion` migration + `rustion/bastion-groups/*` CRUD, GUI `/recordings` page with inline xterm.js asciicast playback + `.rdp-rec` frame walker, Settings → Rustion → Policy panel (global editor + bastion-groups CRUD + force-rustion migration), `useRustionSessionLifecycle` auto-renew hook, SSH + RDP ticket-auth, SSH + RDP listener-loop wire-up with authority/correlation_id stamping, NTLMv2 primitives + CredSSP scaffold validated against MS-NLMP §4.2.4 vectors (Phase 4.2 light), full recording handoff pipeline (Phases 6.1 → 6.5), four-tier policy with `session/open` resolver wiring (Phases 7.1 + 7.2), e2e docker-compose stack. Phase 4.2-full (RC4 sealing + pubKeyAuth + Windows VM), Phase 7.3 (per-tier editors into Resource Types / Asset Groups / Resource Connection pages + full per-resource/AG/type resolver chain), Phase 8 (telemetry + in-GUI replay), Phase 9 (enrolment lifecycle) next. |
