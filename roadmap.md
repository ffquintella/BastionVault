...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1–4 + 3.1 done** (BV 0.7.14 → 0.7.19, Rustion 0.7.11 → 0.7.15): target registry + health, BVRG-v1 envelope build/verify with cross-impl `fips204 ↔ ml-dsa` test, dispatcher, in-memory session table with single-use IP-bound tickets, BV `POST rustion/session/open`, GUI ConnectionProfile `kind`, SSH + RDP ticket-auth modules, **SSH listener-loop wire-up consuming tickets + recording authority/correlation_id stamping + e2e docker-compose scaffold**. Phase 3.2 adds the Dockerfiles + private-key credential kinds; Phase 4.1 adds the ironrdp listener wire-up. Phase 5 (renew/terminate) next. |
