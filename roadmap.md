...
| Rustion Bastion Integration | In progress | [spec](features/rustion-integration.md) — mediates Resource Connect through a PQC bastion; recording delegated to Rustion. **Phases 1 + 2 done** (BV 0.7.14 + 0.7.15, Rustion 0.7.11): target registry, three-strikes health, live HTTP pinger, master-cert config, CLI + GUI Bastions section, BVRG-v1 envelope build (BV) + verify (Rustion), replay LRU, axum `/v1/sessions` + `/v1/health`, cross-impl `fips204` ↔ `ml-dsa` compatibility test. Phase 3 (session materialisation + ticketed SSH proxy + dispatcher) next. |
