 ...
 * share records line up.
 *
 * Everything but the KV-specific bindings below lives in
 * `ObjectSharingCard`, shared with resources and file resources.
 */
  ...
  return (
    <div className="space-y-4">
      ...
      <ObjectSharingCard
        ...
        label={displayPath}
        noun="KV path"
        // KV is the one kind with a real claim endpoint, so any caller
        // may claim an unowned path — the server decides. See
        // `OwnerAdapter`.
        owner={{
          read: api.getKvOwner,
          transfer: api.transferKvOwner,
          claim: api.claimKvOwner,
        }}
        capabilities={["read", "list", "update", "delete", "create"]}
        toast={toast}
        // Long-standing KV behavior: only a literal `root`/`admin`
        // policy sees the ownership controls here, not the wider
        // delegated-admin set the other kinds accept. GUI gating only.
