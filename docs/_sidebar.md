<!-- _sidebar.md — docsify's left-rail navigation. Order here is the
     order operators see in the sidebar, top to bottom. Keep
     getting-started entries first; reference material below; backend
     deep-dives last. -->

- Getting started
  - [Quick start](quick-start.md)
  - [Install](install.md)
  - [Configuration](configuration.md)
  - [CLI reference](cli-reference.md)

- Operations
  - [Administration](administration.md)
  - [Authentication](authentication.md)
  - [API reference](api.md)
  - [Cluster + client discovery](cluster-client-discovery.md)

- Architecture
  - [Design](design.md)
  - [Security structure](security-structure.md)
  - [Cryptography](crypto.md)
  - [Requirements](req.md)

- Secret engines
  - [Overview](secret-engines.md)

- Storage backends
  - [Overview](backend/database/overview.md)
  - [MySQL](backend/database/mysql/mysql.md)
