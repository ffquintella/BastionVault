...
The plugin system already exists ([`features/plugin-system.md`](../features/plugin-system.md), shipped through Phase 5): WASM + out-of-process runtimes, capability-gated host imports, signature verification + quarantine, hot-reload locks, process supervisor, plugin catalog, and metrics. **Dynamic-secret engine plugins reuse all of it unchanged** — they declare a new `kind = "secret/dynamic"` value in the existing manifest schema, request three new dynamic-shaped host capabilities (`dynamic_lease_register`, `dynamic_audit_emit`, `dynamic_pool_checkout`) through the existing capability gate, and otherwise behave exactly like any other plugin in [`plugins-ext/`](../plugins-ext/). No second runtime, no second signing chain, no second catalog. See the "Reuse of the existing plugin engine" section in [`features/dynamic-secrets.md`](../features/dynamic-secrets.md) for the full mapping.
