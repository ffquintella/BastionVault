...
- Scoped ship of the [asset-groups](features/asset-groups.md) design: implements **both the resource and KV-secret halves** of the minimum viable feature — store, reverse indexes, history, resource-delete lifecycle prune, and the ACL qualifier for both target kinds. Pending: KV-delete lifecycle prune (needs a router-level change to see the full logical path), ownership / admin transfer, sharing (blocked on per-user-scoping), and the unified GUI.
